...
SELECTION_SALT = "v37-diagnostic-heldout:500:40-10-20-20-10:v1"
...
def focused_exclusions(path: Path) -> set[tuple[str, int]]:
    ...
    for index, row in enumerate(rows):
        if not isinstance(row, dict) or not isinstance(row.get("source_file"), str) or type(row.get("local_idx")) is not int:
            raise BuildError(f"invalid focused selected_indices[{index}]")
        if row["local_idx"] < 0:
            raise BuildError(f"negative focused local_idx at index {index}")
        result.add((Path(row["source_file"]).name, row["local_idx"]))
    return result
...
def scan_sources(
    source_files: Sequence[tuple[str, list[Path]]], focused: set[tuple[str, int]],
    benchmark_keys: set[str], image_roots: Sequence[Path],
            ...
            for row_group in range(parquet.metadata.num_row_groups):
                ...
                for batch in parquet.iter_batches(
                    ...
                    for batch_index, row in enumerate(batch.to_pylist()):
                        ...
                        if source_label == "11-50" and (parquet_path.name, local_idx) in focused:
...
def build(args: argparse.Namespace) -> Path:
    ...
    focused = focused_exclusions(focused_path)
    ...
    selected, stats = scan_sources(source_files, focused, benchmark_keys, image_roots)
