...
LOG_DIR="/mnt/shared-storage-user/work/EasyR1-latest/logs/train"
mkdir -p ${LOG_DIR}
export output_path='./'
...
export STEPCOUNT_MASKS_METADATA=${STEPCOUNT_MASKS_METADATA:-/mnt/shared-storage-user/StepCount-RL_masks_output/masks_metadata.json}
export STEPCOUNT_MASKS_DIR=${STEPCOUNT_MASKS_DIR:-/mnt/shared-storage-user/StepCount-RL_masks_output/masks}
...
export INTERLEAVED_PER_TURN_MAX_TOKENS=${INTERLEAVED_PER_TURN_MAX_TOKENS:-1500}  # 中间 point 轮最大 token 数
export INTERLEAVED_ANSWER_TURN_MAX_TOKENS=${INTERLEAVED_ANSWER_TURN_MAX_TOKENS:-2500}   # answer 轮最大 token 数
...
export INTERLEAVED_HISTORY_MODE=${INTERLEAVED_HISTORY_MODE:-1} # 1=保留最近1轮历史（与评测对齐），-1=全历史，0=无文本历史，N>0=保留最近N轮
...
# ===================== RL mode switch =====================
# 可选: grpo_standard | drgrpo | gspo | process_reward | dapo
...
case "${STEPCOUNT_RL_MODE}" in
  ...
  gspo)
    ...
    [[ -z "${ACTOR_LR}" ]] && ACTOR_LR="1e-6"
    [[ -z "${ROLLOUT_TEMPERATURE}" ]] && ROLLOUT_TEMPERATURE="1.0"
    [[ -z "${CLIP_RATIO_LOW}" ]] && CLIP_RATIO_LOW="3e-4"  # 0.2, new-standard gspo
  ...
  bok_grpo)
    ...
    export BOK_TAU_INIT=${BOK_TAU_INIT:-0.8}
    ...
    # Per-group std normalization: 0=OFF (Dr.GRPO recommended), 1=ON
...
# - bok_grpo:      ANSWER_WEIGHT=0.8 POINT_WEIGHT=0.15 TRAJECTORY_FORMAT_WEIGHT=0.05
...
MODEL_PATH=/mnt/shared-storage-user/work/LLaMA-Factory-own/LLaMA-Factory/saves/SFT_StepCount_all_with_plus_without_point_reasoning_optimized_no_prompt_answer80_point2p5/checkpoint-3537
...
for required_path in \
    "${MODEL_PATH}" \
    "/mnt/shared-storage-user/work/StepcountModel/dataset/StepCountQA-RL-Traj_0_10" \
    "/mnt/shared-storage-user/work/StepcountModel/dataset/pixmo-test" \
...
LOG_FILE="${LOG_DIR}/training_interleaved_traj_StepCount_mask_reward_v4_${STEPCOUNT_RL_MODE}_${ADV_ESTIMATOR}_ProcessReward${PROCESS_REWARD_ENABLE}_DisableKL${DISABLE_KL}_${RUN_TS}.log"
...
python3 -m verl.trainer.main \
    ...
    data.train_files=/mnt/shared-storage-user/work/StepcountModel/dataset/StepCountQA-RL-Traj_0_10 \
    data.val_files=/mnt/shared-storage-user/work/StepcountModel/dataset/pixmo-test \
    ...
    trainer.experiment_name=StepCount-7B-SFT-30k-high_interleaved_traj_StepCount_mask_reward_v4_${STEPCOUNT_RL_MODE}_${ADV_ESTIMATOR}_ProcessReward${PROCESS_REWARD_ENABLE}_DisableKL${DISABLE_KL}_ailab_$(date +%Y%m%d_%H%M) \
    trainer.logger=['console','wandb'] \
    trainer.save_checkpoint_path=/mnt/shared-storage-user/work/EasyR1-latest/save/StepCount-7B-SFT-30k-high_interleaved_traj_StepCount_mask_reward_v4_${STEPCOUNT_RL_MODE}_${ADV_ESTIMATOR}_ProcessReward${PROCESS_REWARD_ENABLE}_DisableKL${DISABLE_KL}_$(date +%Y%m%d_%H%M) \
