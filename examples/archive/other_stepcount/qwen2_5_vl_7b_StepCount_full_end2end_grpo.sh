...
MODEL_PATH=/weiyang/share/Qwen2.5-VL-7B-Instruct
...
python3 -m verl.trainer.main \
    ...
    trainer.experiment_name=qwen2_5_vl_7b_StepCount_full_end2end_grpo_shuffle_$(date +%Y%m%d_%H%M) \
    trainer.logger=['console','wandb'] \
    trainer.n_gpus_per_node=8 2>&1 | tee "${LOG_DIR}/training_full_end2end_shuffle_$(date +%Y%m%d_%H%M%S).log"