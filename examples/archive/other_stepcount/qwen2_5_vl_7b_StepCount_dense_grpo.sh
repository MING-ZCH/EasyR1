...
python3 -m verl.trainer.main \
    ...
    data.train_files=/weiyang/share/250010212/StepCountQA-RL-Dense \
    ...
    trainer.experiment_name=StepCount-7B-SFT-10k-high_without_point_reasoning_dense_StepCount_mask_reward_v4_grpo_n16_shuffle_$(date +%Y%m%d_%H%M) \
    trainer.logger=['console','wandb'] \
    trainer.save_checkpoint_path=/weiyang/share/250010212/EasyR1/easy_r1/StepCount-7B-SFT-10k-high_without_point_reasoning_dense_StepCount_mask_reward_v4_grpo_n16_shuffle_$(date +%Y%m%d_%H%M) \
    trainer.n_gpus_per_node=8 2>&1 | tee "${LOG_DIR}/training_without_point_reasoning_dense_StepCount_mask_reward_v4_grpo_n16_shuffle_$(date +%Y%m%d_%H%M%S).log"