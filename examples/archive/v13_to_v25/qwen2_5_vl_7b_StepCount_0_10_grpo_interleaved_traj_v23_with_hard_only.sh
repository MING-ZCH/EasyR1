#!/bin/bash
# ================================================================
# v23_with_hard Launch Script — StepCount-7B Interleaved Point-to-Count
# ================================================================
# v23_with_hard = V22 config + ppo_epochs=1 (NaN root-cause fix)
#
# KEY CHANGE from V22:  ppo_epochs=2 → 1
#   Root cause: ppo_epochs=2 causes 17.8% NaN rate (4/5 runs unstable)
#   ppo_epochs=1: 0% NaN over 266 steps (2 runs, V21-R1/R2)
#   See docs/V22_NaN_Definitive_RootCause.md for full evidence.
#
# Inherited from V22 (proven effective):
#   FORMAT_REJECTION=1, GradSpikeProtect@3.0x, max_grad_norm=1.0,
#   KL_coef=0.03, BOK_EASY_THRESHOLD=0.50, BOK_CLIP=4.0,
...
#
# Resume: V21-Run2 step-60 (val/answer=0.7561, ppo_epochs=1, 0 NaN, entropy=0.528)
# ================================================================
#      WINNER_BOOST=0, EASY_SCALE=1.0, QUALITY_BONUS=0
# ================================================================
set -euo pipefail
...
# v23_with_hard: Resume from V21-Run2 step-60 checkpoint (val=0.7561, ppo_epochs=1, 0 NaN, entropy=0.528)
# v23_with_hard: To start fresh from SFT, set v21_RESUME_CKPT="" or comment out the line below
...
export TRAJ_FORMAT_REJECTION=${TRAJ_FORMAT_REJECTION:-1}  # V22+v23_with_hard: ON (prevents BPE d-token collapse)  # 1=format不合格的trajectory reward置0
...
case "${STEPCOUNT_RL_MODE}" in
  ...
  bok_grpo)
    ...
    export BOK_TAU_INIT=${BOK_TAU_INIT:-0.5}
    ...
    export GRAD_SPIKE_THRESHOLD=${GRAD_SPIKE_THRESHOLD:-4.5}   # v22+: 3.5->3.0 (conservative; V21 had 0 NaN with ppo_epochs=1). spike if grad_norm > EMA * threshold
...
MODEL_PATH=/data/workspace/hyleochang/EasyR1-latest/save/StepCount-7B-SFT-30k_v23_mask_reward_v4bok_grpo_hm0_gateoff_bok_grpo_20260410_0037/global_step_213/actor/huggingface
...
for required_path in \
    "${MODEL_PATH}" \
    "/apdcephfs_hldy2/share_305110755/hunyuan/chenhaoz/datasets/StepCountQA-RL-Traj_0_10_hard_only" \
...
LOG_FILE="${LOG_DIR}/training_interleaved_traj_v23_with_hard_StepCount_mask_reward_v4_${STEPCOUNT_RL_MODE}_hm${INTERLEAVED_HISTORY_MODE}_gate${TRAJ_ANSWER_GATE_MODE}_fmtrej${TRAJ_FORMAT_REJECTION}_${ADV_ESTIMATOR}_${RUN_TS}.log"
...
echo "[RunConfig] v23_with_hard: resume_ckpt=${v21_RESUME_CKPT} ppo_epochs=1 fmt_rejection=1 max_grad_norm=1.0 easy_threshold=${BOK_EASY_THRESHOLD:-0.50} easy_score_threshold=${BOK_EASY_SCORE_THRESHOLD:-0.5} filter_all_correct=${BOK_FILTER_ALL_CORRECT:-1}"
...
mkdir -p "${MONITOR_DIR}"
MONITOR_LOG="${MONITOR_DIR}/monitor_v23_with_hard_${RUN_TS}.log"
MONITOR_JSON="${MONITOR_DIR}/metrics_v23_with_hard_${RUN_TS}.jsonl"
export STOP_FILE="/tmp/v23_with_hard_stop_${RUN_TS}"
...
if [[ -f "${MONITOR_SCRIPT}" ]]; then
    ...
    nohup python3 "${MONITOR_SCRIPT}" \
        ...
        > /dev/null 2>"${MONITOR_DIR}/monitor_v23_with_hard_stderr_${RUN_TS}.log" &
    MONITOR_PID=$!
...
python3 -m verl.trainer.main \
    ...
    data.train_files=${STEPCOUNT_TRAIN_DATA:-/apdcephfs_hldy2/share_305110755/hunyuan/chenhaoz/datasets/StepCountQA-RL-Traj_0_10_hard_only} \
    ...
    trainer.experiment_name=StepCount-7B-SFT-30k_v23_with_hard_mask_reward_v4${STEPCOUNT_RL_MODE}_hm${INTERLEAVED_HISTORY_MODE}_gate${TRAJ_ANSWER_GATE_MODE}_fmtrej${TRAJ_FORMAT_REJECTION}_${ADV_ESTIMATOR}_KL${DISABLE_KL}_ailab_$(date +%Y%m%d_%H%M) \
    trainer.logger=['console','wandb'] \
    trainer.save_checkpoint_path=/data/workspace/hyleochang/EasyR1-latest/save/StepCount-7B-SFT-30k_v23_with_hard_mask_reward_v4${STEPCOUNT_RL_MODE}_hm${INTERLEAVED_HISTORY_MODE}_gate${TRAJ_ANSWER_GATE_MODE}_${ADV_ESTIMATOR}_$(date +%Y%m%d_%H%M) \
    trainer.load_checkpoint_path=${v21_RESUME_CKPT} \
    trainer.total_epochs=${STEPCOUNT_TOTAL_EPOCHS:-3} \
