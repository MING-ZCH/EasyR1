#!/bin/bash
# ================================================================
# v24 Launch Script — StepCount-7B Interleaved Point-to-Count
# ================================================================
# V24 = V23 config + smart-filter + LR 1.5e-6 + KL 0.02 (restore V12 effective LR)
#
# KEY CHANGE from V23:
#   1) ACTOR_LR: 1e-6 -> 1.5e-6 (restore V12-level effective LR; safe with ppo_epochs=1)
#   2) kl_coef: 0.03 -> 0.02 (restore V12-level; more exploration under ppo_epochs=1)
#   3) BOK_SMART_FILTER_THRESHOLD: 0 -> 0.955 (release low-point-quality AC groups to Easy)
#   4) TRAJ_ANSWER_GATE_MODE: off (soft gate removed — harms BoK correct-wrong contrast)
#   5) save_freq: 60 -> 30
#
# Why these changes:
#   - V23 reached best val/answer=0.7769 at step 135 with 0 NaN, but pixmo-test=79.77%
#     (V12 best=81.47% at step132). Root cause: V23 effective LR = 33% of V12
#     (LR 1e-6 × ppo_epochs=1 vs V12's 1.5e-6 × ppo_epochs=2)
#   - NaN forensics: ppo_epochs=2 caused NaN in V12(step85), V22(step118);
#     ppo_epochs=1 had 0 NaN across all runs → safe to raise LR with ppo_epochs=1
#   - Smart filter: ~23% of AC groups got zero gradient → release low-point-quality ones
#   - Soft gate analysis: harms BoK correct-wrong contrast (gap shrinks 40%) → keep off
#
# Inherited from V23 (proven effective):
#   FORMAT_REJECTION=1, GradSpikeProtect@3.0x, max_grad_norm=1.0, ppo_epochs=1,
...
#
# Changed from V23: LR 1e-6->1.5e-6, kl_coef 3e-2->2e-2, smart_filter 0->0.955
# Resume: V21-R2 step-60 checkpoint (clean ppo=1 run, val=0.7561, 0 NaN, saves ~2.4h)
#   KL note: ref_policy always = SFT (checkpoint-3537) regardless of resume point.
#   Tau: BOK_TOTAL_STEPS=213 unchanged, progress starts at 60/213≈0.28 (tau≈0.63)
# ================================================================
#      WINNER_BOOST=0, EASY_SCALE=1.0, QUALITY_BONUS=0, SMART_FILTER_TH=0.955
# ================================================================
set -euo pipefail
...
# v24: Resume from V21-R2 step-60 checkpoint.
# Rationale: V21-R2 clean ppo=1 checkpoint, saves ~2.4h vs SFT start. 60-step KL offset tolerable.
# To start fresh from SFT, set v21_RESUME_CKPT="" or comment out the line below.
v21_RESUME_CKPT="${v21_RESUME_CKPT:-/data/workspace/hyleochang/EasyR1-latest/save/StepCount-7B-SFT-30k_v21_mask_reward_v4bok_grpo_hm0_gateoff_bok_grpo_20260406_0311/global_step_60}"
...
export TRAJ_ANSWER_GATE_MODE=${TRAJ_ANSWER_GATE_MODE:-off}  # V24: keep off (soft gate harms BoK correct-wrong contrast, see analysis)
# export TRAJ_SOFT_GATE_BASE=${TRAJ_SOFT_GATE_BASE:-0.8}  # DISABLED in V24: soft gate harmful to BoK correct-wrong contrast
export TRAJ_FORMAT_REJECTION=${TRAJ_FORMAT_REJECTION:-1}  # V22+V23: ON (prevents BPE d-token collapse)  # 1=format不合格的trajectory reward置0
...
case "${STEPCOUNT_RL_MODE}" in
  ...
  bok_grpo)
    ...
    export BOK_TOTAL_STEPS=${BOK_TOTAL_STEPS:-213} # 1 epoch: 13647/64≈213 
    ...
    # V24 Smart Filter: only filter AC groups with group_mean >= threshold (mean_point >= 0.85)
    # Groups below threshold are released to Easy path for point quality optimization.
    # 0 = disabled (V23 behavior: filter ALL all-correct groups).
    export BOK_SMART_FILTER_THRESHOLD=${BOK_SMART_FILTER_THRESHOLD:-0.955}
    # AllWrong Advantage Cap: cap BoK advantages for groups where ALL K answers are wrong.
    # Preserves directional signal from soft_decay but prevents overpowered advantage.
    # 0 = disabled, >0 = max |advantage| for allwrong groups (recommended: 1.0)
    ...
    # Winner Amplification: boost rare-correct advantages by sqrt(K/n_correct).
    # 0 = disabled (default). Recommended: 2.0-3.0 for pass@32->pass@1 focus.
    export BOK_WINNER_BOOST=${BOK_WINNER_BOOST:-0}
    # Easy Gradient Dampening: scale easy_drgrpo advantages by this factor.
    # 1.0 = no change. 0.5 = halve easy gradient to shift budget to BoK critical zone.
    export BOK_EASY_SCALE=${BOK_EASY_SCALE:-1.0}  # V24: full strength (smart filter handles AC precision, no blanket dampening needed)
    # Quality-Ranked BoK: add bonus to correct trajectories with better point quality.
    # 0 = disabled. Recommended: 0.5-1.0 to encourage best-quality correct path.
    export BOK_QUALITY_BONUS=${BOK_QUALITY_BONUS:-0}

    # --- Grad Norm Spike Protection ---
    export GRAD_SPIKE_PROTECT=${GRAD_SPIKE_PROTECT:-1}         # 0=disabled (Cap=1.0 alone sufficient at lr≤1.5e-6); set 1 for lr≥2e-6
    export GRAD_SPIKE_THRESHOLD=${GRAD_SPIKE_THRESHOLD:-3.0}   # v22+: 3.5->3.0 (conservative; V21 had 0 NaN with ppo_epochs=1). spike if grad_norm > EMA * threshold
    export GRAD_SPIKE_COOLDOWN=${GRAD_SPIKE_COOLDOWN:-3}       # v20: 6->3 (bf16)
    export GRAD_SPIKE_LR_FACTOR=${GRAD_SPIKE_LR_FACTOR:-0.1}  # multiply LR by this during cooldown (0.1 = 10x reduction)
    export GRAD_SPIKE_BRAKE_WINDOW=${GRAD_SPIKE_BRAKE_WINDOW:-40}  # v20: window for emergency brake (optimizer steps)
    export GRAD_SPIKE_BRAKE_MAX=${GRAD_SPIKE_BRAKE_MAX:-6}         # v20: max spikes in window before permanent LR halving
    # --- FP16 Gradient Underflow Monitor ---
    export FP16_GRAD_UNDERFLOW_MONITOR=${FP16_GRAD_UNDERFLOW_MONITOR:-0}  # v20: disabled for bf16 (only useful for fp16)
    export FP16_GRAD_UNDERFLOW_MONITOR_EVERY=${FP16_GRAD_UNDERFLOW_MONITOR_EVERY:-10}  # check every N optimizer steps
    ;;
  *)
...
LOG_FILE="${LOG_DIR}/training_interleaved_traj_v24_StepCount_mask_reward_v4_${STEPCOUNT_RL_MODE}_hm${INTERLEAVED_HISTORY_MODE}_gate${TRAJ_ANSWER_GATE_MODE}_fmtrej${TRAJ_FORMAT_REJECTION}_${ADV_ESTIMATOR}_${RUN_TS}.log"
...
echo "[RunConfig] gate_mode=${TRAJ_ANSWER_GATE_MODE} fmt_rej=${TRAJ_FORMAT_REJECTION} consistency_pen=${TRAJ_CONSISTENCY_PENALTY} smart_filter=${BOK_SMART_FILTER_THRESHOLD}"
...
echo "[RunConfig] v24: resume_ckpt=${v21_RESUME_CKPT} ppo_epochs=1 fmt_rejection=1 max_grad_norm=1.0 easy_threshold=${BOK_EASY_THRESHOLD:-0.50} easy_score_threshold=${BOK_EASY_SCORE_THRESHOLD:-0.5} filter_all_correct=${BOK_FILTER_ALL_CORRECT:-1} smart_filter_th=${BOK_SMART_FILTER_THRESHOLD:-0.955}"
echo "[RunConfig] GradSpikeProtect: enabled=${GRAD_SPIKE_PROTECT:-1} threshold=${GRAD_SPIKE_THRESHOLD:-3.0}x cooldown=${GRAD_SPIKE_COOLDOWN:-6} lr_factor=${GRAD_SPIKE_LR_FACTOR:-0.1} brake_window=${GRAD_SPIKE_BRAKE_WINDOW:-40} brake_max=${GRAD_SPIKE_BRAKE_MAX:-6}"
echo "[RunConfig] V3 BoK-GRPO: winner_boost=${BOK_WINNER_BOOST:-0} easy_scale=${BOK_EASY_SCALE:-1.0} quality_bonus=${BOK_QUALITY_BONUS:-0}"


# ── Training Monitor V2: real-time monitoring + health alerts ──
MONITOR_SCRIPT="/data/workspace/hyleochang/EasyR1-latest/tools/monitor_training_v2.py"
MONITOR_DIR="${LOG_DIR}/../monitor"
mkdir -p "${MONITOR_DIR}"
MONITOR_LOG="${MONITOR_DIR}/monitor_v24_${RUN_TS}.log"
MONITOR_JSON="${MONITOR_DIR}/metrics_v24_${RUN_TS}.jsonl"
export STOP_FILE="/tmp/v24_stop_${RUN_TS}"
...
if [[ -f "${MONITOR_SCRIPT}" ]]; then
    ...
    nohup python3 "${MONITOR_SCRIPT}" \
        ...
        > /dev/null 2>"${MONITOR_DIR}/monitor_v24_stderr_${RUN_TS}.log" &
    MONITOR_PID=$!
    ...
    # v20: 1 epoch (fair comparison with V11, which only reached 41% of epoch 1)    
python3 -m verl.trainer.main \
    ...
    trainer.experiment_name=StepCount-7B-SFT-30k_v24_mask_reward_v4${STEPCOUNT_RL_MODE}_hm${INTERLEAVED_HISTORY_MODE}_gate${TRAJ_ANSWER_GATE_MODE}_fmtrej${TRAJ_FORMAT_REJECTION}_${ADV_ESTIMATOR}_KL${DISABLE_KL}_ailab_$(date +%Y%m%d_%H%M) \
    trainer.logger=['console','wandb'] \
    trainer.save_checkpoint_path=/data/workspace/hyleochang/EasyR1-latest/save/StepCount-7B-SFT-30k_v24_mask_reward_v4${STEPCOUNT_RL_MODE}_hm${INTERLEAVED_HISTORY_MODE}_gate${TRAJ_ANSWER_GATE_MODE}_${ADV_ESTIMATOR}_$(date +%Y%m%d_%H%M) \
