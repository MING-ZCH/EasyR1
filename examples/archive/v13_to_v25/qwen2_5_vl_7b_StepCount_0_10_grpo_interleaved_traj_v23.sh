#!/bin/bash
# ================================================================
# v23 Launch Script — StepCount-7B Interleaved Point-to-Count
# ================================================================
# V23 = V22 config + ppo_epochs=1 (NaN root-cause fix)
#
...
# v23: Resume from V21-Run2 step-60 checkpoint (val=0.7561, ppo_epochs=1, 0 NaN, entropy=0.528)
# v23: To start fresh from SFT, set v21_RESUME_CKPT="" or comment out the line below
...
LOG_FILE="${LOG_DIR}/training_interleaved_traj_v23_StepCount_mask_reward_v4_${STEPCOUNT_RL_MODE}_hm${INTERLEAVED_HISTORY_MODE}_gate${TRAJ_ANSWER_GATE_MODE}_fmtrej${TRAJ_FORMAT_REJECTION}_${ADV_ESTIMATOR}_${RUN_TS}.log"
...
echo "[RunConfig] v23: resume_ckpt=${v21_RESUME_CKPT} ppo_epochs=1 fmt_rejection=1 max_grad_norm=1.0 easy_threshold=${BOK_EASY_THRESHOLD:-0.50} easy_score_threshold=${BOK_EASY_SCORE_THRESHOLD:-0.5} filter_all_correct=${BOK_FILTER_ALL_CORRECT:-1}"
...
mkdir -p "${MONITOR_DIR}"
MONITOR_LOG="${MONITOR_DIR}/monitor_v23_${RUN_TS}.log"
MONITOR_JSON="${MONITOR_DIR}/metrics_v23_${RUN_TS}.jsonl"
export STOP_FILE="/tmp/v23_stop_${RUN_TS}"
...
if [[ -f "${MONITOR_SCRIPT}" ]]; then
    ...
    nohup python3 "${MONITOR_SCRIPT}" \
        ...
        > /dev/null 2>"${MONITOR_DIR}/monitor_v23_stderr_${RUN_TS}.log" &
    MONITOR_PID=$!
...
python3 -m verl.trainer.main \
    ...
    trainer.experiment_name=StepCount-7B-SFT-30k_v23_mask_reward_v4${STEPCOUNT_RL_MODE}_hm${INTERLEAVED_HISTORY_MODE}_gate${TRAJ_ANSWER_GATE_MODE}_fmtrej${TRAJ_FORMAT_REJECTION}_${ADV_ESTIMATOR}_KL${DISABLE_KL}_ailab_$(date +%Y%m%d_%H%M) \
    trainer.logger=['console','wandb'] \
    trainer.save_checkpoint_path=/data/workspace/hyleochang/EasyR1-latest/save/StepCount-7B-SFT-30k_v23_mask_reward_v4${STEPCOUNT_RL_MODE}_hm${INTERLEAVED_HISTORY_MODE}_gate${TRAJ_ANSWER_GATE_MODE}_${ADV_ESTIMATOR}_$(date +%Y%m%d_%H%M) \
