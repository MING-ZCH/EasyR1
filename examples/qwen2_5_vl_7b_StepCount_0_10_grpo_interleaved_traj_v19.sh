#!/bin/bash
# ================================================================
# v19 Launch Script — StepCount-7B Interleaved Point-to-Count
# ================================================================
# Based on V18 + FP16 precision + easy_plus_hard dataset:
#   1. Data: easy_plus_hard (13647 rows: 11455 easy + 1616 train-hard×2 + 192 eval-hard×3)
#   2. FP16 precision (actor FSDP + vLLM rollout) — reduces NaN risk vs bf16
#   3. grad_norm: 1.0 (preserves BoK-GRPO signal)
#   4. KL_coef: 0.02 (V12+ proven value)
#   5. BOK_EASY_THRESHOLD: 0.50 (V15/V16 routing)
#   6. BOK_CLIP: 4.0 (V16 stronger hard signal)
#   7. BOK_TAU_INIT: 0.5 (V15/V16 schedule)
#   8. AllWrong Cap=1.0 (new: fixes gradient dominance)
#   9. ANSWER_WEIGHT=0.6, POINT_WEIGHT=0.3
#  10. save_freq: 10, save_limit: -1, val_freq: 15
#  11. ppo_epochs=2, max_grad_norm=1.0
# ================================================================
set -euo pipefail
...
# v19: Resume from V11 S66 checkpoint (comment out to start from SFT)
# v19: Clean start from SFT (uncomment below to resume from V11 S66)
# v19_RESUME_CKPT="${v19_RESUME_CKPT:-/mnt/shared-storage-user/zhangchenhao/work/EasyR1-latest/save/StepCount-7B-SFT-30k_v11_mask_reward_v4bok_grpo_hm0_gateoff_bok_grpo_20260313_1852/global_step_66}"
v19_RESUME_CKPT="${v19_RESUME_CKPT:-}"
...
case "${STEPCOUNT_RL_MODE}" in
  ...
  bok_grpo)
    ...
    export BOK_EASY_THRESHOLD=${BOK_EASY_THRESHOLD:-0.5}
    ...
    export GRAD_SPIKE_COOLDOWN=${GRAD_SPIKE_COOLDOWN:-3}       # reduce LR for N optimizer steps after spike
    ...
    export FP16_GRAD_UNDERFLOW_MONITOR=${FP16_GRAD_UNDERFLOW_MONITOR:-1}  # 1=enabled (monitor zero/subnormal grad ratio)
...
LOG_FILE="${LOG_DIR}/training_interleaved_traj_v19_StepCount_mask_reward_v4_${STEPCOUNT_RL_MODE}_hm${INTERLEAVED_HISTORY_MODE}_gate${TRAJ_ANSWER_GATE_MODE}_fmtrej${TRAJ_FORMAT_REJECTION}_${ADV_ESTIMATOR}_${RUN_TS}.log"
...
echo "[RunConfig] v19: resume_ckpt=${v19_RESUME_CKPT:-none} ppo_epochs=2 max_grad_norm=1.0 easy_threshold=${BOK_EASY_THRESHOLD:-0.50} easy_score_threshold=${BOK_EASY_SCORE_THRESHOLD:-0.5} filter_all_correct=${BOK_FILTER_ALL_CORRECT:-1}"
echo "[RunConfig] GradSpikeProtect: enabled=${GRAD_SPIKE_PROTECT:-0} threshold=${GRAD_SPIKE_THRESHOLD:-3.0}x cooldown=${GRAD_SPIKE_COOLDOWN:-3} lr_factor=${GRAD_SPIKE_LR_FACTOR:-0.1}"
...
# mkdir -p "${MONITOR_DIR}"
# WATCHDOG_LOG="${MONITOR_DIR}/watchdog_v19_${RUN_TS}.log"
# WATCHDOG_JSON="${MONITOR_DIR}/metrics_v19_${RUN_TS}.jsonl"
# export STOP_FILE="/tmp/v19_stop_${RUN_TS}"
...
#         > /dev/null 2>"${MONITOR_DIR}/watchdog_v19_stderr_${RUN_TS}.log" &
#     WATCHDOG_PID=$!
...
# fi
    ...
    # v19: 1 epoch (fair comparison with V11, which only reached 41% of epoch 1)    
python3 -m verl.trainer.main \
    ...
    trainer.experiment_name=StepCount-7B-SFT-30k_v19_mask_reward_v4${STEPCOUNT_RL_MODE}_hm${INTERLEAVED_HISTORY_MODE}_gate${TRAJ_ANSWER_GATE_MODE}_fmtrej${TRAJ_FORMAT_REJECTION}_${ADV_ESTIMATOR}_KL${DISABLE_KL}_ailab_$(date +%Y%m%d_%H%M) \
    trainer.logger=['console','wandb'] \
    trainer.save_checkpoint_path=/mnt/shared-storage-user/zhangchenhao/work/EasyR1-latest/save/StepCount-7B-SFT-30k_v19_mask_reward_v4${STEPCOUNT_RL_MODE}_hm${INTERLEAVED_HISTORY_MODE}_gate${TRAJ_ANSWER_GATE_MODE}_${ADV_ESTIMATOR}_$(date +%Y%m%d_%H%M) \
    trainer.load_checkpoint_path=${v19_RESUME_CKPT:-null} \
