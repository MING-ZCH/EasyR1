#!/bin/bash
# ================================================================
# V12-fix Launch Script — StepCount-7B Interleaved Point-to-Count
# ================================================================
# V12-fix = V12 exact params(LR=1.0e-6, clip=0.5, ppo=2, kl=0.03) + data(easy+hard(hard x2)) + safety features
#
# EXPERIMENTAL DESIGN:
...
#
# KEY CONFIG:
#   LR=1.0e-6, clip=0.5, ppo=2, kl=0.03, easy+hard(hard x2), SFT cold start
...
LOG_FILE="${LOG_DIR}/training_interleaved_traj_v12fix_${STEPCOUNT_RL_MODE}_${ADV_ESTIMATOR}_${RUN_TS}.log"
...
mkdir -p "${MONITOR_DIR}"
MONITOR_LOG="${MONITOR_DIR}/monitor_v12fix_${RUN_TS}.log"
MONITOR_JSON="${MONITOR_DIR}/metrics_v12fix_${RUN_TS}.jsonl"
export STOP_FILE="/tmp/v12fix_stop_${RUN_TS}"
rm -f "${STOP_FILE}"
...
if [[ -f "${MONITOR_SCRIPT}" ]]; then
    ...
    nohup python3 "${MONITOR_SCRIPT}" \
        ...
        > /dev/null 2>"${MONITOR_DIR}/monitor_v12fix_stderr_${RUN_TS}.log" &
    MONITOR_PID=$!
...
python3 -m verl.trainer.main \
    ...
    trainer.experiment_name=StepCount-7B-SFT-30k_v12fix_signGD_mixed_${ADV_ESTIMATOR}_$(date +%Y%m%d_%H%M) \
    trainer.logger=['console','wandb'] \
    trainer.save_checkpoint_path=/mnt/shared-storage-user/zhangchenhao/work/EasyR1-latest/save/StepCount-7B-SFT-30k_v12fix_signGD_mixed_${ADV_ESTIMATOR}_$(date +%Y%m%d_%H%M) \
