...
case "${STEPCOUNT_RL_MODE}" in
  bok_grpo)
    ...
    [[ -z "${ACTOR_LR}" ]] && ACTOR_LR="2e-6"     # Config B': sign-GD + highest LR
    ...
    export BOK_CLIP=${BOK_CLIP:-4.0}              # Config B': wider clip for hard
    ...
    export GRAD_SPIKE_ABSOLUTE_CAP=${GRAD_SPIKE_ABSOLUTE_CAP:-4.0}  # NEW: absolute cap
...
LOG_FILE="${LOG_DIR}/training_interleaved_traj_v12Bp_${STEPCOUNT_RL_MODE}_${ADV_ESTIMATOR}_${RUN_TS}.log"
...
mkdir -p "${MONITOR_DIR}"
MONITOR_LOG="${MONITOR_DIR}/monitor_v12Bp_${RUN_TS}.log"
MONITOR_JSON="${MONITOR_DIR}/metrics_v12Bp_${RUN_TS}.jsonl"
export STOP_FILE="/tmp/v12Bp_stop_${RUN_TS}"
rm -f "${STOP_FILE}"
...
if [[ -f "${MONITOR_SCRIPT}" ]]; then
    ...
    nohup python3 "${MONITOR_SCRIPT}" \
        ...
        > /dev/null 2>"${MONITOR_DIR}/monitor_v12Bp_stderr_${RUN_TS}.log" &
    MONITOR_PID=$!
...
python3 -m verl.trainer.main \
    ...
    trainer.experiment_name=StepCount-7B-SFT-30k_v12Bp_signGD_mixed_${ADV_ESTIMATOR}_$(date +%Y%m%d_%H%M) \
    trainer.logger=['console','wandb'] \
    trainer.save_checkpoint_path=/mnt/shared-storage-user/zhangchenhao/work/EasyR1-latest/save/StepCount-7B-SFT-30k_v12Bp_signGD_mixed_${ADV_ESTIMATOR}_$(date +%Y%m%d_%H%M) \
