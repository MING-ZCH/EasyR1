#!/bin/bash
# ================================================================
# V27 Launch Script — StepCount-7B Interleaved Point-to-Count
# (sparse-guarded: strict-JSON fix + threshold=3.0 + no abs_cap)
# ================================================================
# V27 = v26 + 3 targeted fixes based on root-cause analysis
#
...
#
# 3 FIXES vs v26:
#   1. TRAJ_POINT_STRICT_JSON=1 (DEFAULT in reward code)
#      → JSON解析失败return None，彻底关闭reward hacking漏洞
#      → v26的9.8%漂移率→0%，format_reward全程≥0.97
...
#
# UNCHANGED from v26:
#   - ACTOR_LR=1.5e-6  (v25验证峰值0.7826，修复后格式安全)
#   - GPU: 4xH200 140GB → gpu_util=0.65
#   - BOK_CLIP=4.0      (v23_SC锚点，NaN=0)
#   - ppo_epochs=1, kl=0.03, max_grad_norm=1.0
#   - answer_w=0.6, point_w=0.3, format_w=0.1
#
# DATA: easy_plus_hard (default) | easy-only (via env var)
...
#   - peak val_ans >= 0.7826 (beat v25)           MEDIUM confidence
...
export TRAJ_POINT_STRICT_JSON=${TRAJ_POINT_STRICT_JSON:-1}  # V27 core fix: strict JSON in _parse_pred_point
...
case "${STEPCOUNT_RL_MODE}" in
  bok_grpo)
    ...
    export BOK_TOTAL_STEPS=${BOK_TOTAL_STEPS:-213}  # V27: easy+hard 13641/64≈213; set 179 for easy-only
...
for required_path in \
    "${MODEL_PATH}" \
    "${STEPCOUNT_TRAIN_DATA:-/mnt/shared-storage-user/zhangchenhao/work/StepcountModel/dataset/StepCountQA-RL-Traj_0_10_easy_plus_hard}" \
...
LOG_FILE="${LOG_DIR}/training_interleaved_traj_v27_${STEPCOUNT_RL_MODE}_${ADV_ESTIMATOR}_${RUN_TS}.log"

echo "================================================================"
echo "[V27 RunConfig] Sparse-Guarded (STRICT_JSON + thresh=3.0 + no abs_cap)"
echo "================================================================"
...
mkdir -p "${MONITOR_DIR}"
MONITOR_LOG="${MONITOR_DIR}/monitor_v27_${RUN_TS}.log"
MONITOR_JSON="${MONITOR_DIR}/metrics_v27_${RUN_TS}.jsonl"
...
if [[ -f "${MONITOR_SCRIPT}" ]]; then
    ...
    nohup python3 "${MONITOR_SCRIPT}" \
        ...
        > /dev/null 2>"${MONITOR_DIR}/monitor_v27_stderr_${RUN_TS}.log" &
    MONITOR_PID=$!
...
python3 -m verl.trainer.main \
    ...
    trainer.experiment_name=StepCount-7B-SFT-30k_v27_sparse_guarded_${ADV_ESTIMATOR}_$(date +%Y%m%d_%H%M) \
    trainer.logger=['console','wandb'] \
    trainer.save_checkpoint_path=/mnt/shared-storage-user/zhangchenhao/work/EasyR1-latest/save/StepCount-7B-SFT-30k_v27_sparse_guarded_${ADV_ESTIMATOR}_$(date +%Y%m%d_%H%M) \
...
echo "[V27] Training finished successfully."
