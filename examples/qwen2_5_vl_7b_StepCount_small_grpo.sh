...
MODEL_PATH=/weiyang/250010212/checkpoints/StepCount-7b-SFT-16k-without-reasoning

python3 -m verl.trainer.main \
    config=examples/config.yaml \
    data.train_files=/weiyang/share/250010212/StepCountQA-RL-Small \
    ...
    trainer.experiment_name=StepCount-7B-SFT-10k-high_without_point_reasoning_small_StepCount_mask_reward_v4_grpo_n16_shuffle_$(date +%Y%m%d_%H%M) \
    trainer.logger=['console','wandb'] \
    trainer.total_epochs=1 \
    trainer.save_checkpoint_path=/weiyang/share/250010212/EasyR1/easy_r1/StepCount-7B-SFT-10k-high_without_point_reasoning_small_StepCount_mask_reward_v4_grpo_n16_shuffle_$(date +%Y%m%d_%H%M) \
    trainer.n_gpus_per_node=8 2>&1 | tee "${LOG_DIR}/training_without_point_reasoning_small_StepCount_mask_reward_v4_grpo_n16_shuffle_$(date +%Y%m%d_%H%M%S).log"