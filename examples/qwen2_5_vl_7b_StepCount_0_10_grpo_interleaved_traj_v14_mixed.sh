#!/bin/bash
# ================================================================
# V14-mixed Launch Script — Mixed Hard+Easy Dataset — StepCount-7B Interleaved Point-to-Count
# ================================================================
# Key changes from V14 (mixed hard+easy experiment):
#   1. Training data: mixed (5792 = 1808 hard + 384 eval_extra + 3600 easy, weighted by error rate)
#   2. total_epochs=1 (~91 steps/epoch, V14-hard proved multi-epoch harmful)
#   3. BOK_TOTAL_STEPS=91 (matches 1 epoch)
#   4. save_freq=20, val_freq=10 (~4-5 checkpoints, frequent validation)
#   5. All other params same as V14 (LR=1.5e-6, BOK_CLIP=3.0, max_grad_norm=1.0)
#   6. NaN hypothesis: hard-data reduces steps + LR=1.5e-6 should be stable
#   7. RL hypothesis: all data is informative → stronger gradient signal per step
# ================================================================
...
# [DISABLED with gate_mode=off] Hard gate threshold (only active when gate_mode=hard)
export TRAJ_ANSWER_GATE_THRESHOLD=${TRAJ_ANSWER_GATE_THRESHOLD:-0.4}
# [DISABLED with gate_mode=off] Soft gate base (only active when gate_mode=soft)
export TRAJ_SOFT_GATE_BASE=${TRAJ_SOFT_GATE_BASE:-0.8}
# [DISABLED] Format rejection (OFF since V11; 1=reject format-failed trajectories)
export TRAJ_FORMAT_REJECTION=${TRAJ_FORMAT_REJECTION:-0}
...
case "${STEPCOUNT_RL_MODE}" in
  ...
  bok_grpo)
    ...
    [[ -z "${PROCESS_REWARD_ENABLE}" ]] && PROCESS_REWARD_ENABLE="0"
    [[ -z "${ACTOR_LR}" ]] && ACTOR_LR="1.5e-6"  # V12 proven: 1.5e-6 stable for effective training
    ...
    export BOK_CLIP=${BOK_CLIP:-3.0}   # V14: keep 3.0 (LR reduction is primary fix)
    ...
    export BOK_TOTAL_STEPS=${BOK_TOTAL_STEPS:-91} # matches total_epochs=1
    ...
    # ---- Currently Disabled (preserved for future experiments) ----
    # [DISABLED] Per-group advantage normalization (Dr.GRPO paper: OFF better; P2 improvement may test ON)
    export BOK_ADV_NORMALIZE=${BOK_ADV_NORMALIZE:-0}
    # [DISABLED] Anti-collapse uniform mix (rarely triggers collapsed=0-8/1024; code default=0.1 sufficient)
    export BOK_UNIFORM_MIX=${BOK_UNIFORM_MIX:-0.1}
    # [DISABLED] Logit cap safety (redundant with tau_adaptive; code default=12.0)
    export BOK_LOGIT_CAP=${BOK_LOGIT_CAP:-12.0}
    # [DISABLED] Adaptive tau safety net (never activates tau_bumped=0/1024; code default=1)
    export BOK_TAU_ADAPTIVE=${BOK_TAU_ADAPTIVE:-1}
    export BOK_TAU_MIN=${BOK_TAU_MIN:-0.08}
    # [DISABLED] DAPO dynamic sampling (OFF since V11: death spiral risk with format_rejection)
    export BOK_DAPO_FILTER=${BOK_DAPO_FILTER:-0}
    export BOK_DAPO_AUTO_DISABLE_THRESHOLD=${BOK_DAPO_AUTO_DISABLE_THRESHOLD:-0.5}
    # [DISABLED] Min batch std for clip_std fallback (irrelevant with fallback=drgrpo)
...
LOG_FILE="${LOG_DIR}/training_interleaved_traj_v14_mixed_StepCount_mask_reward_v4_${STEPCOUNT_RL_MODE}_hm${INTERLEAVED_HISTORY_MODE}_gate${TRAJ_ANSWER_GATE_MODE}_fmtrej${TRAJ_FORMAT_REJECTION}_${ADV_ESTIMATOR}_${RUN_TS}.log"
...
echo "[RunConfig] gate_mode=${TRAJ_ANSWER_GATE_MODE} gate_thresh=${TRAJ_ANSWER_GATE_THRESHOLD} soft_base=${TRAJ_SOFT_GATE_BASE} fmt_rej=${TRAJ_FORMAT_REJECTION} consistency_pen=${TRAJ_CONSISTENCY_PENALTY}"
...
echo "[RunConfig] bok_fallback=${BOK_FALLBACK_MODE:-zscore} dapo_filter=${BOK_DAPO_FILTER:-0} adv_norm=${BOK_ADV_NORMALIZE:-0}"
...
mkdir -p "${MONITOR_DIR}"
WATCHDOG_LOG="${MONITOR_DIR}/watchdog_v14_mixed_${RUN_TS}.log"
WATCHDOG_JSON="${MONITOR_DIR}/metrics_v14_mixed_${RUN_TS}.jsonl"
STOP_FILE="/tmp/v14_mixed_stop_${RUN_TS}"
...
if [[ -f "${WATCHDOG_SCRIPT}" ]]; then
    ...
    nohup python3 "${WATCHDOG_SCRIPT}" \
        ...
        > /dev/null 2>"${MONITOR_DIR}/watchdog_v14_mixed_stderr_${RUN_TS}.log" &
    WATCHDOG_PID=$!
...
python3 -m verl.trainer.main \
    ...
    data.train_files=/mnt/shared-storage-user/zhangchenhao/work/StepcountModel/dataset/StepCountQA-RL-Traj_0_10_mixed_hard_easy \
    ...
    trainer.experiment_name=StepCount-7B-SFT-30k_v14_mixed_mask_reward_v4${STEPCOUNT_RL_MODE}_hm${INTERLEAVED_HISTORY_MODE}_gate${TRAJ_ANSWER_GATE_MODE}_fmtrej${TRAJ_FORMAT_REJECTION}_${ADV_ESTIMATOR}_KL${DISABLE_KL}_ailab_$(date +%Y%m%d_%H%M) \
    trainer.logger=['console','wandb'] \
    trainer.save_checkpoint_path=/mnt/shared-storage-user/zhangchenhao/work/EasyR1-latest/save/StepCount-7B-SFT-30k_v14_mixed_mask_reward_v4${STEPCOUNT_RL_MODE}_hm${INTERLEAVED_HISTORY_MODE}_gate${TRAJ_ANSWER_GATE_MODE}_${ADV_ESTIMATOR}_$(date +%Y%m%d_%H%M) \
