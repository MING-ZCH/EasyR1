#!/bin/bash
# ================================================================
# v22 Launch Script — StepCount-7B Interleaved Point-to-Count
# ================================================================
# Based on V12 recipe + V20 dataset + evidence-based parameter analysis:
#   1. Data: easy_plus_hard (13647 rows, same as V20)
#   2. BF16 precision (both V12 and V20 verified bf16)
#   3. GradSpikeProtect: threshold=3.0x (v22: 3.5->3.0, V18 proven: 1 NaN), cooldown=3
#   4. max_grad_norm: 1.0 (ELR=1e-6, safe window ~65 steps)
#   5. KL_coef: 0.03 (V21: increased from 0.02 for stronger policy anchoring)
#   6. BOK_EASY_THRESHOLD: 0.50 (TH=0.50, validates mixed-data routing)
#      TH=0.50+mixed → 22% BoK + 69% DrGRPO (natural balance from bimodal data)
#      [V12 used TH=0.75 because easy-only data made TH=0.50→0.1% BoK; not applicable here]
#   7. BOK_CLIP: 4.0 (BoK advantage range [-2.5, +4.0])
#   8. BOK_TAU_INIT: 0.7 → 0.3 cosine (V12 τ schedule, KEY CHANGE from V20)
#   9. AllWrong Cap=1.0 (prevents wrong-answer positive advantage = correct-answer)
#  10. ANSWER_WEIGHT=0.6, POINT_WEIGHT=0.3, FORMAT_WEIGHT=0.1
#  11. save_freq=60, val_freq=60 (213 total steps)
#  12. ppo_epochs=2 (actor), FORMAT_REJECTION=1 (v22 fix), DAPO dual clip (0.2/0.28)
#  13. V3 params DISABLED (analysis proved unnecessary):
...
# v22: Resume from V21-Run1 step-60 checkpoint (healthy: format_fail<1%, answer=0.75, ppo_epochs=2 match)
# v22: To start fresh from SFT, set v21_RESUME_CKPT="" or comment out the line below
v21_RESUME_CKPT="${v21_RESUME_CKPT:-/mnt/shared-storage-user/zhangchenhao/work/EasyR1-latest/save/StepCount-7B-SFT-30k_v21_mask_reward_v4bok_grpo_hm0_gateoff_bok_grpo_20260404_0428/global_step_60}"
...
export TRAJ_FORMAT_REJECTION=${TRAJ_FORMAT_REJECTION:-1}  # V22: ON (creates 7.5x format signal gap; prevents BPE d-token collapse)  # 1=format不合格的trajectory reward置0
...
case "${STEPCOUNT_RL_MODE}" in
  ...
  bok_grpo)
    ...
    export GRAD_SPIKE_THRESHOLD=${GRAD_SPIKE_THRESHOLD:-3.0}   # v22: 3.5->3.0 (V18 proven: 1 NaN; V21@3.5: 14 NaN). spike if grad_norm > EMA * threshold
...
LOG_FILE="${LOG_DIR}/training_interleaved_traj_v22_StepCount_mask_reward_v4_${STEPCOUNT_RL_MODE}_hm${INTERLEAVED_HISTORY_MODE}_gate${TRAJ_ANSWER_GATE_MODE}_fmtrej${TRAJ_FORMAT_REJECTION}_${ADV_ESTIMATOR}_${RUN_TS}.log"
...
echo "[RunConfig] v22: resume_ckpt=${v21_RESUME_CKPT} ppo_epochs=2 fmt_rejection=1 max_grad_norm=1.0 easy_threshold=${BOK_EASY_THRESHOLD:-0.50} easy_score_threshold=${BOK_EASY_SCORE_THRESHOLD:-0.5} filter_all_correct=${BOK_FILTER_ALL_CORRECT:-1}"
...
mkdir -p "${MONITOR_DIR}"
MONITOR_LOG="${MONITOR_DIR}/monitor_v22_${RUN_TS}.log"
MONITOR_JSON="${MONITOR_DIR}/metrics_v22_${RUN_TS}.jsonl"
export STOP_FILE="/tmp/v22_stop_${RUN_TS}"
...
if [[ -f "${MONITOR_SCRIPT}" ]]; then
    ...
    nohup python3 "${MONITOR_SCRIPT}" \
        ...
        > /dev/null 2>"${MONITOR_DIR}/monitor_v22_stderr_${RUN_TS}.log" &
    MONITOR_PID=$!
...
python3 -m verl.trainer.main \
    ...
    trainer.experiment_name=StepCount-7B-SFT-30k_v22_mask_reward_v4${STEPCOUNT_RL_MODE}_hm${INTERLEAVED_HISTORY_MODE}_gate${TRAJ_ANSWER_GATE_MODE}_fmtrej${TRAJ_FORMAT_REJECTION}_${ADV_ESTIMATOR}_KL${DISABLE_KL}_ailab_$(date +%Y%m%d_%H%M) \
    trainer.logger=['console','wandb'] \
    trainer.save_checkpoint_path=/mnt/shared-storage-user/zhangchenhao/work/EasyR1-latest/save/StepCount-7B-SFT-30k_v22_mask_reward_v4${STEPCOUNT_RL_MODE}_hm${INTERLEAVED_HISTORY_MODE}_gate${TRAJ_ANSWER_GATE_MODE}_${ADV_ESTIMATOR}_$(date +%Y%m%d_%H%M) \
