#!/bin/bash
# ================================================================
# v18 Launch Script — StepCount-7B Interleaved Point-to-Count
# ================================================================
# Based on V16 + improvements from V14-V16 analysis:
#   1. Data: easy-only (StepCountQA-RL-Traj_0_10) — env-overridable
#   2. LR: 1e-6 (default; env-overridable via ACTOR_LR)
...
# v18: Resume from V11 S66 checkpoint (comment out to start from SFT)
# v18: Clean start from SFT (uncomment below to resume from V11 S66)
# v18_RESUME_CKPT="${v18_RESUME_CKPT:-/mnt/shared-storage-user/zhangchenhao/work/EasyR1-latest/save/StepCount-7B-SFT-30k_v11_mask_reward_v4bok_grpo_hm0_gateoff_bok_grpo_20260313_1852/global_step_66}"
v18_RESUME_CKPT="${v18_RESUME_CKPT:-}"
...
case "${STEPCOUNT_RL_MODE}" in
  ...
  bok_grpo)
    ...
    export BOK_TOTAL_STEPS=${BOK_TOTAL_STEPS:-90} # 1 epoch: 178 
    ...
    export GRAD_SPIKE_THRESHOLD=${GRAD_SPIKE_THRESHOLD:-3.0}   # spike if grad_norm > EMA * threshold
...
LOG_FILE="${LOG_DIR}/training_interleaved_traj_v18_StepCount_mask_reward_v4_${STEPCOUNT_RL_MODE}_hm${INTERLEAVED_HISTORY_MODE}_gate${TRAJ_ANSWER_GATE_MODE}_fmtrej${TRAJ_FORMAT_REJECTION}_${ADV_ESTIMATOR}_${RUN_TS}.log"
...
echo "[RunConfig] v18: resume_ckpt=${v18_RESUME_CKPT:-none} ppo_epochs=2 max_grad_norm=1.0 easy_threshold=${BOK_EASY_THRESHOLD:-0.50} easy_score_threshold=${BOK_EASY_SCORE_THRESHOLD:-0.5} filter_all_correct=${BOK_FILTER_ALL_CORRECT:-1}"
...
mkdir -p "${MONITOR_DIR}"
WATCHDOG_LOG="${MONITOR_DIR}/watchdog_v18_${RUN_TS}.log"
WATCHDOG_JSON="${MONITOR_DIR}/metrics_v18_${RUN_TS}.jsonl"
export STOP_FILE="/tmp/v18_stop_${RUN_TS}"
...
if [[ -f "${WATCHDOG_SCRIPT}" ]]; then
    ...
    nohup python3 "${WATCHDOG_SCRIPT}" \
        ...
        > /dev/null 2>"${MONITOR_DIR}/watchdog_v18_stderr_${RUN_TS}.log" &
    WATCHDOG_PID=$!
    ...
    # v18: 1 epoch (fair comparison with V11, which only reached 41% of epoch 1)    
python3 -m verl.trainer.main \
    config=${CONFIG_PATH} \
    algorithm.adv_estimator=${ADV_ESTIMATOR} \
    algorithm.disable_kl=${DISABLE_KL} \
    algorithm.kl_coef=2e-2 \
    data.train_files=${STEPCOUNT_TRAIN_DATA:-/mnt/shared-storage-user/zhangchenhao/work/StepcountModel/dataset/StepCountQA-RL-Traj_0_10_mixed_hard_easy} \
    ...
    trainer.experiment_name=StepCount-7B-SFT-30k_v18_mask_reward_v4${STEPCOUNT_RL_MODE}_hm${INTERLEAVED_HISTORY_MODE}_gate${TRAJ_ANSWER_GATE_MODE}_fmtrej${TRAJ_FORMAT_REJECTION}_${ADV_ESTIMATOR}_KL${DISABLE_KL}_ailab_$(date +%Y%m%d_%H%M) \
    trainer.logger=['console','wandb'] \
    trainer.save_checkpoint_path=/mnt/shared-storage-user/zhangchenhao/work/EasyR1-latest/save/StepCount-7B-SFT-30k_v18_mask_reward_v4${STEPCOUNT_RL_MODE}_hm${INTERLEAVED_HISTORY_MODE}_gate${TRAJ_ANSWER_GATE_MODE}_${ADV_ESTIMATOR}_$(date +%Y%m%d_%H%M) \
    trainer.load_checkpoint_path=${v18_RESUME_CKPT:-null} \
