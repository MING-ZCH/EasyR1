#!/bin/bash
# ================================================================
# V12-Update Launch Script — StepCount-7B Interleaved Point-to-Count
# ================================================================
...
case "${STEPCOUNT_RL_MODE}" in
  bok_grpo)
    ...
    [[ -z "${ACTOR_LR}" ]] && ACTOR_LR="1.5e-6"      # V12 original LR
    ...
    export BOK_CLIP=${BOK_CLIP:-3.0}    # return to v12
...
LOG_FILE="${LOG_DIR}/training_interleaved_traj_v12update_${STEPCOUNT_RL_MODE}_${ADV_ESTIMATOR}_${RUN_TS}.log"
...
mkdir -p "${MONITOR_DIR}"
MONITOR_LOG="${MONITOR_DIR}/monitor_v12update_${RUN_TS}.log"
MONITOR_JSON="${MONITOR_DIR}/metrics_v12update_${RUN_TS}.jsonl"
export STOP_FILE="/tmp/v12update_stop_${RUN_TS}"
rm -f "${STOP_FILE}"
...
if [[ -f "${MONITOR_SCRIPT}" ]]; then
    ...
    nohup python3 "${MONITOR_SCRIPT}" \
        ...
        > /dev/null 2>"${MONITOR_DIR}/monitor_v12update_stderr_${RUN_TS}.log" &
    MONITOR_PID=$!
...
python3 -m verl.trainer.main \
    ...
    trainer.experiment_name=StepCount-7B-SFT-30k_v12update_signGD_mixed_hardx2_${ADV_ESTIMATOR}_$(date +%Y%m%d_%H%M) \
    trainer.logger=['console','wandb'] \
    trainer.save_checkpoint_path=/mnt/shared-storage-user/zhangchenhao/work/EasyR1-latest/save/StepCount-7B-SFT-30k_v12update_signGD_mixed_hardx2_${ADV_ESTIMATOR}_$(date +%Y%m%d_%H%M) \
...
echo "[V12-Update] Training finished successfully."
