...
#   3. GradSpikeProtect: threshold=4.0x, cooldown=3
...
# v22: Fresh start from SFT (no resume) checkpoint (comment out to start from SFT)
# v22: Uncomment below to resume from a checkpoint (uncomment below to resume from V11 S66)
v22_RESUME_CKPT="${v22_RESUME_CKPT:-}"
# v22_RESUME_CKPT="${v22_RESUME_CKPT:-}"
...
echo "[RunConfig] v22: resume_ckpt=${v22_RESUME_CKPT} ppo_epochs=2 max_grad_norm=1.0 easy_threshold=${BOK_EASY_THRESHOLD:-0.50} easy_score_threshold=${BOK_EASY_SCORE_THRESHOLD:-0.5} filter_all_correct=${BOK_FILTER_ALL_CORRECT:-1}"
...
python3 -m verl.trainer.main \
    ...
    trainer.load_checkpoint_path=${v22_RESUME_CKPT} \
    trainer.total_epochs=3 \
    trainer.save_freq=56 \
    trainer.save_limit=-1 \
    trainer.val_freq=30 \
    trainer.n_gpus_per_node=2 2>&1 | tee "${LOG_FILE}"
