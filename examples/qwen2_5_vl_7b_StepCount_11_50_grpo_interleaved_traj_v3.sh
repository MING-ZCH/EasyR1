#!/bin/bash
# ================================================================
# V3 Launch Script — StepCount-7B Interleaved Point-to-Count (11-50 range)
# ================================================================
# Based on V2 (11-50) + aligned with V17 (0-10) proven parameters:
#   1. ADD BOK_ALLWRONG_CAP=1.0 (critical fix: eliminates AllWrong gradient spike)
#   2. ADD BOK_ALLWRONG_ANSWER_THRESHOLD=0.5
#   3. ADD GRAD_SPIKE_PROTECT=0 (with params, enable via env if needed)
#   4. kl_coef: 3e-2 → 2e-2 (align V17, stronger constraint)
#   5. ROLLOUT_TEMPERATURE: 0.7 → 1.0 (align V17, more diverse rollouts)
#   6. lr_warmup_ratio: 0.1 → 0.05 (align V17)
#   7. TRAJ_CONSISTENCY_PENALTY: 0.75 → 0.5 (align V17)
#   8. TRAJ_EXTRA_POINT_PENALTY_LAMBDA: 0.5 → 1.0 (align V17, stronger overcounting penalty)
#   9. BOK_LOW_VAR_THRESHOLD: 3e-4 → 1e-5 (align V17)
#  10. save_freq: 50 → 30 (more frequent checkpoints)
#   Key unchanged: LR=1e-6, max_grad_norm=1.0, MAX_TURNS=51, ppo_epochs=2
#   11-50 specific: ANSWER_WEIGHT=0.6, POINT_WEIGHT=0.3 (align V17)
# ================================================================
...
# V12: Resume from V11 S66 checkpoint (comment out to start from SFT)
# V12: Clean start from SFT (uncomment below to resume from V11 S66)
# V15_RESUME_CKPT="${V15_RESUME_CKPT:-/mnt/shared-storage-user/zhangchenhao/work/EasyR1-latest/save/StepCount-7B-SFT-30k_v11_mask_reward_v4bok_grpo_hm0_gateoff_bok_grpo_20260313_1852/global_step_66}"
V3_RESUME_CKPT="${V3_RESUME_CKPT:-}"
...
export STEPCOUNT_MASKS_METADATA=${STEPCOUNT_MASKS_METADATA:-/mnt/shared-storage-user/zhangchenhao/work/StepcountModel/dataset/sam_mask/StepCount-RL_masks_output/masks_metadata.json}
export STEPCOUNT_MASKS_DIR=${STEPCOUNT_MASKS_DIR:-/mnt/shared-storage-user/zhangchenhao/work/StepcountModel/dataset/sam_mask/StepCount-RL_masks_output/masks}
...
export INTERLEAVED_MAX_TURNS=${INTERLEAVED_MAX_TURNS:-51}   # 最大轮数（50 轮 point + 1 轮 answer）
export INTERLEAVED_PER_TURN_MAX_TOKENS=${INTERLEAVED_PER_TURN_MAX_TOKENS:-6000}  # 中间 point 轮最大 token 数
export INTERLEAVED_ANSWER_TURN_MAX_TOKENS=${INTERLEAVED_ANSWER_TURN_MAX_TOKENS:-6000}   # answer 轮最大 token 数
...
case "${STEPCOUNT_RL_MODE}" in
  ...
  bok_grpo)
    ...
    [[ -z "${PROCESS_REWARD_ENABLE}" ]] && PROCESS_REWARD_ENABLE="0"
    [[ -z "${ACTOR_LR}" ]] && ACTOR_LR="1.0e-6"  # V15-A: 1.5e-6 → 5e-7 (eff_intensity=4.0×5e-7×2=4.0e-6 SAFE)
    ...
    export BOK_CLIP=${BOK_CLIP:-4.0}   # V15-A: 3.0 → 4.0 (unlock hard BoK signal, Q3 conclusion)
    ...
    export BOK_TAU_INIT=${BOK_TAU_INIT:-0.5}  # V15-A: 0.7 → 0.5 (BoK only handles hard ≤8/16 now, more selective start)
    ...
    # export BOK_TOTAL_STEPS=${BOK_TOTAL_STEPS:-90} # V15: matches total_epochs=1 with mixed_hard_easy dataset
    # Per-group advantage normalization: 0=disabled (default, per Dr.GRPO finding)
    export BOK_ADV_NORMALIZE=${BOK_ADV_NORMALIZE:-0}
    export BOK_LOGIT_CAP=${BOK_LOGIT_CAP:-12.0}  # V14-mixed: prevent softmax overflow
    ...
    export BOK_EASY_THRESHOLD=${BOK_EASY_THRESHOLD:-0.50}  # V15-A: 0.75 → 0.50 (optimal BoK/DrGRPO crossover, Q1 conclusion)
    ...
    export GRAD_SPIKE_THRESHOLD=${GRAD_SPIKE_THRESHOLD:-4.0}   # spike if grad_norm > EMA * threshold
...
MODEL_PATH=/mnt/shared-storage-user/zhangchenhao/work/EasyR1-latest/save/StepCount-7B-SFT-30k_v12_mask_reward_v4bok_grpo_hm0_gateoff_bok_grpo_20260315_0136/global_step_178/actor/huggingface
...
for required_path in \
    "${MODEL_PATH}" \
    "/mnt/shared-storage-user/zhangchenhao/work/StepcountModel/dataset/StepCountQA-RL-Traj_0_10_mixed_hard_easy" \
    "/mnt/shared-storage-user/zhangchenhao/work/StepcountModel/dataset/stepcount-500" \
...
LOG_FILE="${LOG_DIR}/training_interleaved_traj_11_50_v3_StepCount_mask_reward_v4_${STEPCOUNT_RL_MODE}_hm${INTERLEAVED_HISTORY_MODE}_gate${TRAJ_ANSWER_GATE_MODE}_fmtrej${TRAJ_FORMAT_REJECTION}_${ADV_ESTIMATOR}_${RUN_TS}.log"
...
echo "[RunConfig] v3: resume_ckpt=${V3_RESUME_CKPT:-none} ppo_epochs=2 max_grad_norm=1.0 easy_threshold=${BOK_EASY_THRESHOLD:-0.50} easy_score_threshold=${BOK_EASY_SCORE_THRESHOLD:-0.5} filter_all_correct=${BOK_FILTER_ALL_CORRECT:-1}"
echo "[RunConfig] allwrong_cap=${BOK_ALLWRONG_CAP:-off} allwrong_threshold=${BOK_ALLWRONG_ANSWER_THRESHOLD:-0.5} grad_spike_protect=${GRAD_SPIKE_PROTECT:-0}"
# ── V11 Watchdog: 自动监控 + 异常预警 ──
WATCHDOG_SCRIPT="/mnt/shared-storage-user/zhangchenhao/work/EasyR1-latest/tools/monitor_v11_watchdog.py"
MONITOR_DIR="${LOG_DIR}/../monitor/11_50"
mkdir -p "${MONITOR_DIR}"
WATCHDOG_LOG="${MONITOR_DIR}/watchdog_v3_${RUN_TS}.log"
WATCHDOG_JSON="${MONITOR_DIR}/metrics_v3_${RUN_TS}.jsonl"
STOP_FILE="/tmp/v1_stop_${RUN_TS}"
...
if [[ -f "${WATCHDOG_SCRIPT}" ]]; then
    ...
    nohup python3 "${WATCHDOG_SCRIPT}" \
        ...
        > /dev/null 2>"${MONITOR_DIR}/watchdog_v3_stderr_${RUN_TS}.log" &
    WATCHDOG_PID=$!
...
python3 -m verl.trainer.main \
    ...
    data.train_files=/mnt/shared-storage-user/zhangchenhao/work/StepcountModel/dataset/StepCountQA-RL-Traj_11_50 \
    data.system_prompt_file=${SYSTEM_PROMPT_FILE} \
    data.format_prompt=null \
    data.val_files=/mnt/shared-storage-user/zhangchenhao/work/StepcountModel/dataset/stepcount-500 \
    ...
    trainer.experiment_name=StepCount-7B-SFT-30k_trajectory_11_50_v3_mask_reward_v4${STEPCOUNT_RL_MODE}_hm${INTERLEAVED_HISTORY_MODE}_gate${TRAJ_ANSWER_GATE_MODE}_fmtrej${TRAJ_FORMAT_REJECTION}_${ADV_ESTIMATOR}_KL${DISABLE_KL}_ailab_$(date +%Y%m%d_%H%M) \
    trainer.logger=['console','wandb'] \
    trainer.save_checkpoint_path=/mnt/shared-storage-user/zhangchenhao/work/EasyR1-latest/save/StepCount-7B-SFT-30k_trajectory_11_50_v3_mask_reward_v4${STEPCOUNT_RL_MODE}_hm${INTERLEAVED_HISTORY_MODE}_gate${TRAJ_ANSWER_GATE_MODE}_${ADV_ESTIMATOR}_$(date +%Y%m%d_%H%M) \
