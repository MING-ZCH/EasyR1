#!/bin/bash
# ================================================================
# V12-A Launch Script — StepCount-7B Interleaved Point-to-Count
# ================================================================
# V26-B = V12 sign-GD(clip=0.5) + confirmed optimizations(ppo=1,kl=0.03) + mixed data(hard x2)
#
# EXPERIMENTAL DESIGN:
...
#
# KEY CONFIG:
#   LR=2.5e-6, clip=0.5, ppo=1, kl=0.03, easy+hard(hard x2), SFT cold start
#   Per-step eff = 2.5e-6 * 0.5 = 1.25e-6 (98% of V12's 1.275e-6)
#   Total budget = 213 * 1.25e-6 = 266e-6 (240% of V12)
...
# V26-B: SFT cold start, no resume
# v21_RESUME_CKPT=""
...
case "${STEPCOUNT_RL_MODE}" in
  bok_grpo)
    ...
    [[ -z "${ACTOR_LR}" ]] && ACTOR_LR="1e-6"        # V26-B Config A: V23-proven safe LR
    ...
    export BOK_SMART_FILTER_THRESHOLD=${BOK_SMART_FILTER_THRESHOLD:-0}  # V24: release low-point AC groups
    ...
    export GRAD_SPIKE_ABSOLUTE_CAP=${GRAD_SPIKE_ABSOLUTE_CAP:-5.0}  # NEW: absolute cap
    export FP16_GRAD_UNDERFLOW_MONITOR=${FP16_GRAD_UNDERFLOW_MONITOR:-0}
    ;;
  *)
    echo "[FATAL] V26-B only supports bok_grpo mode. Got: ${STEPCOUNT_RL_MODE}" >&2
    exit 1
    ;;
esac
...
LOG_FILE="${LOG_DIR}/training_interleaved_traj_v12A_${STEPCOUNT_RL_MODE}_${ADV_ESTIMATOR}_${RUN_TS}.log"

echo "================================================================"
echo "[V12-A RunConfig] Sign-GD + Mixed Data (hard x2) + Confirmed Optimizations"
echo "================================================================"
...
mkdir -p "${MONITOR_DIR}"
MONITOR_LOG="${MONITOR_DIR}/monitor_v12A_${RUN_TS}.log"
MONITOR_JSON="${MONITOR_DIR}/metrics_v12A_${RUN_TS}.jsonl"
export STOP_FILE="/tmp/v12A_stop_${RUN_TS}"
rm -f "${STOP_FILE}"
...
if [[ -f "${MONITOR_SCRIPT}" ]]; then
    ...
    nohup python3 "${MONITOR_SCRIPT}" \
        ...
        > /dev/null 2>"${MONITOR_DIR}/monitor_v12A_stderr_${RUN_TS}.log" &
    MONITOR_PID=$!
...
python3 -m verl.trainer.main \
    ...
    data.train_files=${STEPCOUNT_TRAIN_DATA:-/mnt/shared-storage-user/zhangchenhao/work/StepcountModel/dataset/StepCountQA-RL-Traj_0_10} \
    ...
    trainer.experiment_name=StepCount-7B-SFT-30k_v12A_easy_baseline_${ADV_ESTIMATOR}_$(date +%Y%m%d_%H%M) \
    trainer.logger=['console','wandb'] \
    trainer.save_checkpoint_path=/mnt/shared-storage-user/zhangchenhao/work/EasyR1-latest/save/StepCount-7B-SFT-30k_v12A_easy_baseline_${ADV_ESTIMATOR}_$(date +%Y%m%d_%H%M) \
...
echo "[V12-A] Training finished successfully."
