...
case "${STEPCOUNT_RL_MODE}" in
  ...
  bok_grpo)
    ...
    export BOK_TAU_FINAL=${BOK_TAU_FINAL:-0.3}  # anneal to selective (low τ); avoid <0.5 to preserve format compliance
    ...
    export BOK_TOTAL_STEPS=${BOK_TOTAL_STEPS:-256} # 1 epoch: 178 
...
LOG_FILE="${LOG_DIR}/training_interleaved_traj_v11_StepCount_mask_reward_v4_${STEPCOUNT_RL_MODE}_hm${INTERLEAVED_HISTORY_MODE}_gate${TRAJ_ANSWER_GATE_MODE}_fmtrej${TRAJ_FORMAT_REJECTION}_${ADV_ESTIMATOR}_${RUN_TS}.log"
...
echo "[RunConfig] V11: ppo_epochs=2 max_grad_norm=0.5 easy_threshold=${BOK_EASY_THRESHOLD:-0.75} easy_score_threshold=${BOK_EASY_SCORE_THRESHOLD:-0.5} filter_all_correct=${BOK_FILTER_ALL_CORRECT:-1}"
...
mkdir -p "${MONITOR_DIR}"
WATCHDOG_LOG="${MONITOR_DIR}/watchdog_v11_${RUN_TS}.log"
WATCHDOG_JSON="${MONITOR_DIR}/metrics_v11_${RUN_TS}.jsonl"
STOP_FILE="/tmp/v11_stop_${RUN_TS}"
...
python3 -m verl.trainer.main \
    ...
    trainer.experiment_name=StepCount-7B-SFT-30k_v11_mask_reward_v4${STEPCOUNT_RL_MODE}_hm${INTERLEAVED_HISTORY_MODE}_gate${TRAJ_ANSWER_GATE_MODE}_fmtrej${TRAJ_FORMAT_REJECTION}_${ADV_ESTIMATOR}_KL${DISABLE_KL}_ailab_$(date +%Y%m%d_%H%M) \
    trainer.logger=['console','wandb'] \
    trainer.save_checkpoint_path=/mnt/shared-storage-user/zhangchenhao/work/EasyR1-latest/save/StepCount-7B-SFT-30k_v11_mask_reward_v4${STEPCOUNT_RL_MODE}_hm${INTERLEAVED_HISTORY_MODE}_gate${TRAJ_ANSWER_GATE_MODE}_${ADV_ESTIMATOR}_$(date +%Y%m%d_%H%M) \
