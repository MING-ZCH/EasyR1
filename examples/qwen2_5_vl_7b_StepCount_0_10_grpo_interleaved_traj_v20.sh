#!/bin/bash
# ================================================================
# v20 Launch Script — StepCount-7B Interleaved Point-to-Count
# ================================================================
# Based on V12-best config + comprehensive BoK-GRPO analysis:
#   1. Data: easy_plus_hard (13647 rows: 11455 easy + 1616 train-hard×2 + 192 eval-hard×3)
#   2. BF16 precision (V19 FP16 → NaN; bf16 rollback for stability)
#   3. GradSpikeProtect v2: EMA cooldown=6 + emergency brake window=40
#   4. max_grad_norm: 1.0 (grad_norm typically 1.3-2.4, always clipped)
#   5. KL_coef: 0.02 (V12+ proven value)
#   6. BOK_EASY_THRESHOLD: 0.50 ← KEY: optimal for mixed data (bimodal n distribution)
...
#   7. BOK_CLIP: 4.0 (BoK advantage range [-0.9, +4.0])
#   8. BOK_TAU_INIT: 0.5 → 0.3 cosine schedule
#   9. AllWrong Cap=1.0 (prevents allwrong gradient dominance)
#  10. ANSWER_WEIGHT=0.6, POINT_WEIGHT=0.3, FORMAT_WEIGHT=0.1
#  11. save_freq=15, save_limit=-1, val_freq=15
#  12. ppo_epochs=2, DAPO dual clip (0.2/0.28)
...
# v20: Resume from V11 S66 checkpoint (comment out to start from SFT)
# v20: Clean start from SFT (uncomment below to resume from V11 S66)
v20_RESUME_CKPT="${v20_RESUME_CKPT:-/mnt/shared-storage-user/zhangchenhao/work/EasyR1-latest/save/StepCount-7B-SFT-30k_v20_mask_reward_v4bok_grpo_hm0_gateoff_bok_grpo_20260401_2226/global_step_60}"
# v20_RESUME_CKPT="${v20_RESUME_CKPT:-}"
...
LOG_FILE="${LOG_DIR}/training_interleaved_traj_v20_StepCount_mask_reward_v4_${STEPCOUNT_RL_MODE}_hm${INTERLEAVED_HISTORY_MODE}_gate${TRAJ_ANSWER_GATE_MODE}_fmtrej${TRAJ_FORMAT_REJECTION}_${ADV_ESTIMATOR}_${RUN_TS}.log"
...
echo "[RunConfig] v19: resume_ckpt=${v20_RESUME_CKPT} ppo_epochs=2 max_grad_norm=1.0 easy_threshold=${BOK_EASY_THRESHOLD:-0.50} easy_score_threshold=${BOK_EASY_SCORE_THRESHOLD:-0.5} filter_all_correct=${BOK_FILTER_ALL_CORRECT:-1}"
echo "[RunConfig] GradSpikeProtect: enabled=${GRAD_SPIKE_PROTECT:-1} threshold=${GRAD_SPIKE_THRESHOLD:-3.5}x cooldown=${GRAD_SPIKE_COOLDOWN:-6} lr_factor=${GRAD_SPIKE_LR_FACTOR:-0.1} brake_window=${GRAD_SPIKE_BRAKE_WINDOW:-40} brake_max=${GRAD_SPIKE_BRAKE_MAX:-6}"
...
mkdir -p "${MONITOR_DIR}"
MONITOR_LOG="${MONITOR_DIR}/monitor_v20_${RUN_TS}.log"
MONITOR_JSON="${MONITOR_DIR}/metrics_v20_${RUN_TS}.jsonl"
export STOP_FILE="/tmp/v20_stop_${RUN_TS}"
...
if [[ -f "${MONITOR_SCRIPT}" ]]; then
    ...
    nohup python3 "${MONITOR_SCRIPT}" \
        ...
        > /dev/null 2>"${MONITOR_DIR}/monitor_v20_stderr_${RUN_TS}.log" &
    MONITOR_PID=$!
...
python3 -m verl.trainer.main \
    ...
    trainer.experiment_name=StepCount-7B-SFT-30k_v20_mask_reward_v4${STEPCOUNT_RL_MODE}_hm${INTERLEAVED_HISTORY_MODE}_gate${TRAJ_ANSWER_GATE_MODE}_fmtrej${TRAJ_FORMAT_REJECTION}_${ADV_ESTIMATOR}_KL${DISABLE_KL}_ailab_$(date +%Y%m%d_%H%M) \
    trainer.logger=['console','wandb'] \
    trainer.save_checkpoint_path=/mnt/shared-storage-user/zhangchenhao/work/EasyR1-latest/save/StepCount-7B-SFT-30k_v20_mask_reward_v4${STEPCOUNT_RL_MODE}_hm${INTERLEAVED_HISTORY_MODE}_gate${TRAJ_ANSWER_GATE_MODE}_${ADV_ESTIMATOR}_$(date +%Y%m%d_%H%M) \
    trainer.load_checkpoint_path=${v20_RESUME_CKPT} \
