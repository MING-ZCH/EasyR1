#!/bin/bash
# ================================================================
# V2 Launch Script â StepCount-7B Interleaved Point-to-Count (11-50 range)
# ================================================================
# Based on V1 (11-50) with the following optimizations:
#   1. Val dataset: pixmo-test(0-10) â stepcount-500(11-50) for proper val monitoring
#   2. ppo_epochs: 1 â 2 (match V12 best, more gradient steps per rollout)
#   3. ANSWER_WEIGHT: 0.6 â 0.5, POINT_WEIGHT: 0.3 â 0.4 (emphasize point accuracy for dense scenes)
#   4. save_freq: 30 â 20 (more frequent saves, match V12)
#   5. kl_coef: 3e-2 â 2e-2 (match V12, allow more exploration)
#   Key unchanged: LR=1.5e-6, max_grad_norm=1.0, MAX_TURNS=51, BOK params
# ================================================================
...
export TRAJ_CONSISTENCY_PENALTY=${TRAJ_CONSISTENCY_PENALTY:-0.75} # 对point与answer不一致样本额外乘以惩罚系数
...
export TRAJ_EXTRA_POINT_PENALTY_LAMBDA=${TRAJ_EXTRA_POINT_PENALTY_LAMBDA:-0.5}  # V11-fix: penalize extra points beyond GT to eliminate overcounting bias
...
case "${STEPCOUNT_RL_MODE}" in
  ...
  bok_grpo)
    ...
    [[ -z "${ROLLOUT_TEMPERATURE}" ]] && ROLLOUT_TEMPERATURE="0.7"  # v8: keep 1.0 for controlled experiment (seed fix provides diversity)
    [[ -z "${CLIP_RATIO_LOW}" ]] && CLIP_RATIO_LOW="0.2"
    [[ -z "${CLIP_RATIO_HIGH}" ]] && CLIP_RATIO_HIGH="0.28"
    [[ -z "${CLIP_RATIO_DUAL}" ]] && CLIP_RATIO_DUAL="3.0"
    [[ -z "${DISABLE_KL}" ]] && DISABLE_KL="false"
...
export ANSWER_WEIGHT=${ANSWER_WEIGHT:-0.5}  # V11: 0.7->0.6 (V10 point_mean declined 0.94->0.85, increase point gradient)
export POINT_WEIGHT=${POINT_WEIGHT:-0.4}  # V11: 0.2->0.3 (compensate for point degradation in V10)
...
LOG_FILE="${LOG_DIR}/training_interleaved_traj_11_50_v2_StepCount_mask_reward_v4_${STEPCOUNT_RL_MODE}_hm${INTERLEAVED_HISTORY_MODE}_gate${TRAJ_ANSWER_GATE_MODE}_fmtrej${TRAJ_FORMAT_REJECTION}_${ADV_ESTIMATOR}_${RUN_TS}.log"
...
echo "[RunConfig] v2: resume_ckpt=${v1_RESUME_CKPT:-none} ppo_epochs=2 max_grad_norm=1.0 easy_threshold=${BOK_EASY_THRESHOLD:-0.50} easy_score_threshold=${BOK_EASY_SCORE_THRESHOLD:-0.5} filter_all_correct=${BOK_FILTER_ALL_CORRECT:-1}"
...
mkdir -p "${MONITOR_DIR}"
WATCHDOG_LOG="${MONITOR_DIR}/watchdog_v1_${RUN_TS}.log"
WATCHDOG_JSON="${MONITOR_DIR}/metrics_v1_${RUN_TS}.jsonl"
...
if [[ -f "${WATCHDOG_SCRIPT}" ]]; then
    ...
    nohup python3 "${WATCHDOG_SCRIPT}" \
        ...
        > /dev/null 2>"${MONITOR_DIR}/watchdog_v1_stderr_${RUN_TS}.log" &
    WATCHDOG_PID=$!
...
python3 -m verl.trainer.main \
    ...
    trainer.experiment_name=StepCount-7B-SFT-30k_trajectory_11_50_v2_mask_reward_v4${STEPCOUNT_RL_MODE}_hm${INTERLEAVED_HISTORY_MODE}_gate${TRAJ_ANSWER_GATE_MODE}_fmtrej${TRAJ_FORMAT_REJECTION}_${ADV_ESTIMATOR}_KL${DISABLE_KL}_ailab_$(date +%Y%m%d_%H%M) \
    trainer.logger=['console','wandb'] \
    trainer.save_checkpoint_path=/mnt/shared-storage-user/zhangchenhao/work/EasyR1-latest/save/StepCount-7B-SFT-30k_trajectory_11_50_v2_mask_reward_v4${STEPCOUNT_RL_MODE}_hm${INTERLEAVED_HISTORY_MODE}_gate${TRAJ_ANSWER_GATE_MODE}_${ADV_ESTIMATOR}_$(date +%Y%m%d_%H%M) \
