#!/bin/bash
# ================================================================
# V17 Launch Script — StepCount-7B Interleaved Point-to-Count
# ================================================================
...
# V17: Resume from V11 S66 checkpoint (comment out to start from SFT)
# V17: Clean start from SFT (uncomment below to resume from V11 S66)
# V17_RESUME_CKPT="${V17_RESUME_CKPT:-/mnt/shared-storage-user/zhangchenhao/work/EasyR1-latest/save/StepCount-7B-SFT-30k_v11_mask_reward_v4bok_grpo_hm0_gateoff_bok_grpo_20260313_1852/global_step_66}"
V17_RESUME_CKPT="${V17_RESUME_CKPT:-}"
...
case "${STEPCOUNT_RL_MODE}" in
  ...
  bok_grpo)
    ...
    export BOK_TOTAL_STEPS=${BOK_TOTAL_STEPS:-178} # 1 epoch: 178 
    ...
    export GRAD_SPIKE_PROTECT=${GRAD_SPIKE_PROTECT:-0}         # 0=disabled (Cap=1.0 alone sufficient at lr≤1.5e-6); set 1 for lr≥2e-6
    export GRAD_SPIKE_THRESHOLD=${GRAD_SPIKE_THRESHOLD:-5.0}   # spike if grad_norm > EMA * threshold
...
LOG_FILE="${LOG_DIR}/training_interleaved_traj_v17_StepCount_mask_reward_v4_${STEPCOUNT_RL_MODE}_hm${INTERLEAVED_HISTORY_MODE}_gate${TRAJ_ANSWER_GATE_MODE}_fmtrej${TRAJ_FORMAT_REJECTION}_${ADV_ESTIMATOR}_${RUN_TS}.log"
...
echo "[RunConfig] V17: resume_ckpt=${V17_RESUME_CKPT:-none} ppo_epochs=2 max_grad_norm=1.0 easy_threshold=${BOK_EASY_THRESHOLD:-0.50} easy_score_threshold=${BOK_EASY_SCORE_THRESHOLD:-0.5} filter_all_correct=${BOK_FILTER_ALL_CORRECT:-1}"
echo "[RunConfig] GradSpikeProtect: enabled=${GRAD_SPIKE_PROTECT:-0} threshold=${GRAD_SPIKE_THRESHOLD:-5.0}x cooldown=${GRAD_SPIKE_COOLDOWN:-3} lr_factor=${GRAD_SPIKE_LR_FACTOR:-0.1}"
...
mkdir -p "${MONITOR_DIR}"
WATCHDOG_LOG="${MONITOR_DIR}/watchdog_v17_${RUN_TS}.log"
WATCHDOG_JSON="${MONITOR_DIR}/metrics_v17_${RUN_TS}.jsonl"
export STOP_FILE="/tmp/v17_stop_${RUN_TS}"
...
if [[ -f "${WATCHDOG_SCRIPT}" ]]; then
    ...
    nohup python3 "${WATCHDOG_SCRIPT}" \
        ...
        > /dev/null 2>"${MONITOR_DIR}/watchdog_v17_stderr_${RUN_TS}.log" &
    WATCHDOG_PID=$!
    ...
    # V17: 1 epoch (fair comparison with V11, which only reached 41% of epoch 1)    
python3 -m verl.trainer.main \
    ...
    trainer.experiment_name=StepCount-7B-SFT-30k_v17_mask_reward_v4${STEPCOUNT_RL_MODE}_hm${INTERLEAVED_HISTORY_MODE}_gate${TRAJ_ANSWER_GATE_MODE}_fmtrej${TRAJ_FORMAT_REJECTION}_${ADV_ESTIMATOR}_KL${DISABLE_KL}_ailab_$(date +%Y%m%d_%H%M) \
    trainer.logger=['console','wandb'] \
    trainer.save_checkpoint_path=/mnt/shared-storage-user/zhangchenhao/work/EasyR1-latest/save/StepCount-7B-SFT-30k_v17_mask_reward_v4${STEPCOUNT_RL_MODE}_hm${INTERLEAVED_HISTORY_MODE}_gate${TRAJ_ANSWER_GATE_MODE}_${ADV_ESTIMATOR}_$(date +%Y%m%d_%H%M) \
    trainer.load_checkpoint_path=${V17_RESUME_CKPT:-null} \
