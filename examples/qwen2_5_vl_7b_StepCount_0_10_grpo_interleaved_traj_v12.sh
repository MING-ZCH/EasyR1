#!/bin/bash
# ================================================================
# V12 Launch Script — StepCount-7B Interleaved Point-to-Count
# ================================================================
# Based on V11 with the following changes:
#   1. Fix1 (Extra Point Penalty) + Fix2 (GT-Scaled Alpha) enabled
#   2. Clean start from SFT (fair comparison with V11)
#   3. KL coef kept at 0.02 (same as V11 for fair comparison)
#   4. LR kept at 1.5e-6 (same as V11 for fair comparison)
#   5. save_freq=20 for more frequent checkpointing
#   7. total_epochs=1 (V11 only reached 41% of epoch 1, fair comparison)
#   6. BoK tau kept at 0.7 (same as V11, seamless continuity at S66)
# ================================================================
set -euo pipefail
...
# V12_RESUME_CKPT="${V12_RESUME_CKPT:-/mnt/shared-storage-user/zhangchenhao/work/EasyR1-latest/save/StepCount-7B-SFT-30k_v11_mask_reward_v4bok_grpo_hm0_gateoff_bok_grpo_20260313_1852/global_step_66}"
V12_RESUME_CKPT="${V12_RESUME_CKPT:-}"
...
case "${STEPCOUNT_RL_MODE}" in
  ...
  bok_grpo)
    ...
    [[ -z "${PROCESS_REWARD_ENABLE}" ]] && PROCESS_REWARD_ENABLE="0"
    [[ -z "${ACTOR_LR}" ]] && ACTOR_LR="1.5e-6"  # V12: same as V11 for fair comparison
    ...
    export BOK_TAU_INIT=${BOK_TAU_INIT:-0.7}  # V12: keep V11 schedule (tau=0.667 at S66, seamless continuation)   # start with mild exploration (τ=0.8)
...
LOG_FILE="${LOG_DIR}/training_interleaved_traj_v12_StepCount_mask_reward_v4_${STEPCOUNT_RL_MODE}_hm${INTERLEAVED_HISTORY_MODE}_gate${TRAJ_ANSWER_GATE_MODE}_fmtrej${TRAJ_FORMAT_REJECTION}_${ADV_ESTIMATOR}_${RUN_TS}.log"
...
echo "[RunConfig] V12: resume_ckpt=${V12_RESUME_CKPT:-none} ppo_epochs=2 max_grad_norm=0.5 easy_threshold=${BOK_EASY_THRESHOLD:-0.75} easy_score_threshold=${BOK_EASY_SCORE_THRESHOLD:-0.5} filter_all_correct=${BOK_FILTER_ALL_CORRECT:-1}"
...
mkdir -p "${MONITOR_DIR}"
WATCHDOG_LOG="${MONITOR_DIR}/watchdog_v12_${RUN_TS}.log"
WATCHDOG_JSON="${MONITOR_DIR}/metrics_v12_${RUN_TS}.jsonl"
STOP_FILE="/tmp/v12_stop_${RUN_TS}"
...
if [[ -f "${WATCHDOG_SCRIPT}" ]]; then
    ...
    nohup python3 "${WATCHDOG_SCRIPT}" \
        ...
        > /dev/null 2>"${MONITOR_DIR}/watchdog_v12_stderr_${RUN_TS}.log" &
    WATCHDOG_PID=$!
...
python3 -m verl.trainer.main \
    ...
    trainer.experiment_name=StepCount-7B-SFT-30k_v12_mask_reward_v4${STEPCOUNT_RL_MODE}_hm${INTERLEAVED_HISTORY_MODE}_gate${TRAJ_ANSWER_GATE_MODE}_fmtrej${TRAJ_FORMAT_REJECTION}_${ADV_ESTIMATOR}_KL${DISABLE_KL}_ailab_$(date +%Y%m%d_%H%M) \
    trainer.logger=['console','wandb'] \
    trainer.save_checkpoint_path=/mnt/shared-storage-user/zhangchenhao/work/EasyR1-latest/save/StepCount-7B-SFT-30k_v12_mask_reward_v4${STEPCOUNT_RL_MODE}_hm${INTERLEAVED_HISTORY_MODE}_gate${TRAJ_ANSWER_GATE_MODE}_${ADV_ESTIMATOR}_$(date +%Y%m%d_%H%M) \
    trainer.load_checkpoint_path=${V12_RESUME_CKPT:-null} \
