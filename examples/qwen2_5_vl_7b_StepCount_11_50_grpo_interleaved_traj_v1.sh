#!/bin/bash
# ================================================================
# V1-A Launch Script — StepCount-7B Interleaved Point-to-Count
# ================================================================
# Based on V16 with the following parameter optimization:
#   1. BOK_CLIP: 3.0 → 4.0 (unlock 1-3/16 BoK hard-problem signal)
#   2. BOK_EASY_THRESHOLD: 0.75 → 0.50 (optimal routing crossover)
#   3. BOK_TAU_INIT: 0.7 → 0.5 (more selective for pure-hard regime)
#   4. ACTOR_LR: 1.5e-6 → 5e-7 (eff_intensity budget: 4.0×5e-7×2=4.0e-6 SAFE)
#   5. max_grad_norm: 0.5 → 1.0 (V14-validated, prevents over-clipping)
#   Other params unchanged from V12 (ppo_epochs=2, kl_coef=0.02, PPO ε=0.2/0.28)
#
#   Safety verification: eff_intensity = 4.0e-6 (1.07× V7 baseline 3.75e-6) ✅
# ================================================================
...
case "${STEPCOUNT_RL_MODE}" in
  ...
  bok_grpo)
    ...
    [[ -z "${PROCESS_REWARD_ENABLE}" ]] && PROCESS_REWARD_ENABLE="0"
    [[ -z "${ACTOR_LR}" ]] && ACTOR_LR="1.5e-6"  # V15-A: 1.5e-6 → 5e-7 (eff_intensity=4.0×5e-7×2=4.0e-6 SAFE)
...
LOG_FILE="${LOG_DIR}/training_interleaved_traj_11_50_v1_StepCount_mask_reward_v4_${STEPCOUNT_RL_MODE}_hm${INTERLEAVED_HISTORY_MODE}_gate${TRAJ_ANSWER_GATE_MODE}_fmtrej${TRAJ_FORMAT_REJECTION}_${ADV_ESTIMATOR}_${RUN_TS}.log"
...
echo "[RunConfig] v1: resume_ckpt=${v1_RESUME_CKPT:-none} ppo_epochs=1 max_grad_norm=1.0 easy_threshold=${BOK_EASY_THRESHOLD:-0.50} easy_score_threshold=${BOK_EASY_SCORE_THRESHOLD:-0.5} filter_all_correct=${BOK_FILTER_ALL_CORRECT:-1}"
...
python3 -m verl.trainer.main \
    ...
    trainer.experiment_name=StepCount-7B-SFT-30k_trajectory_11_50_v1_mask_reward_v4${STEPCOUNT_RL_MODE}_hm${INTERLEAVED_HISTORY_MODE}_gate${TRAJ_ANSWER_GATE_MODE}_fmtrej${TRAJ_FORMAT_REJECTION}_${ADV_ESTIMATOR}_KL${DISABLE_KL}_ailab_$(date +%Y%m%d_%H%M) \
    trainer.logger=['console','wandb'] \
    trainer.save_checkpoint_path=/mnt/shared-storage-user/zhangchenhao/work/EasyR1-latest/save/StepCount-7B-SFT-30k_trajectory_11_50_v1_mask_reward_v4${STEPCOUNT_RL_MODE}_hm${INTERLEAVED_HISTORY_MODE}_gate${TRAJ_ANSWER_GATE_MODE}_${ADV_ESTIMATOR}_$(date +%Y%m%d_%H%M) \
