#!/bin/bash
# ================================================================
# V12-B' Launch Script — StepCount-7B Interleaved Point-to-Count
# ================================================================
# V12-Update = V12 exact params(LR=1.5e-6, clip=0.5, ppo=2, kl=0.02) + data(easy+hard, hard x2) + safety features
#
# EXPERIMENTAL DESIGN:
#   Test: "Does clip=0.5 (sign-GD) cure mixed data gradient noise?"
#   All V14-V25 mixed experiments used clip=1.0 -> easy-only advantage might be clip-dependent
#
# KEY CONFIG:
#   LR=1.5e-6, clip=0.5, ppo=2, kl=0.02, easy+hard(hard x2), SFT cold start
#   Per-step eff = 1.5e-6 * 0.5 = 7.5e-7 (V12 per-pass match, 59% per-step)
#   Total budget = 213 * 7.5e-7 = 160e-6 (144% of V12)
#   NO RESUME (avoids config override bug from checkpoint resume)
# ================================================================
set -euo pipefail
...
# V12-Update: SFT cold start, no resume
# v21_RESUME_CKPT=""
...
export TRAIN_HARD_OVERSAMPLE_FACTOR=${TRAIN_HARD_OVERSAMPLE_FACTOR:-1}  # Config B: disabled (natural ~28% hard ratio)
...
case "${STEPCOUNT_RL_MODE}" in
  bok_grpo)
    ...
    [[ -z "${ACTOR_LR}" ]] && ACTOR_LR="1.5e-6"     # Config B': sign-GD + highest LR
    ...
    export BOK_CLIP=${BOK_CLIP:-3.0}              # Config B': wider clip for hard
    ...
    export BOK_ADV_NORMALIZE=${BOK_ADV_NORMALIZE:-1}
    ...
    export BOK_SMART_FILTER_THRESHOLD=${BOK_SMART_FILTER_THRESHOLD:-0.955} # V24
    ...
    export GRAD_SPIKE_LR_FACTOR=${GRAD_SPIKE_LR_FACTOR:-0.3}
    export GRAD_SPIKE_BRAKE_WINDOW=${GRAD_SPIKE_BRAKE_WINDOW:-40}
    export GRAD_SPIKE_BRAKE_MAX=${GRAD_SPIKE_BRAKE_MAX:-6}
    export GRAD_SPIKE_ABSOLUTE_CAP=${GRAD_SPIKE_ABSOLUTE_CAP:-3.0}  # NEW: absolute cap
export GRAD_NONFINITE_COOLDOWN=${GRAD_NONFINITE_COOLDOWN:-8}
export GRAD_NONFINITE_LR_FACTOR=${GRAD_NONFINITE_LR_FACTOR:-0.3}
export GRAD_NONFINITE_BRAKE_WINDOW=${GRAD_NONFINITE_BRAKE_WINDOW:-20}
export GRAD_NONFINITE_BRAKE_MAX=${GRAD_NONFINITE_BRAKE_MAX:-2}
    ...
    ;;
  *)
    echo "[FATAL] V12-Update only supports bok_grpo mode. Got: ${STEPCOUNT_RL_MODE}" >&2
    exit 1
    ;;
esac
...
LOG_FILE="${LOG_DIR}/training_interleaved_traj_v12Bpfix_${STEPCOUNT_RL_MODE}_${ADV_ESTIMATOR}_${RUN_TS}.log"

echo "================================================================"
echo "[V12-Update RunConfig] Sign-GD + Mixed Data (hard x2) + Confirmed Optimizations"
echo "================================================================"
echo "[RunConfig] model=${MODEL_PATH}"
echo "[RunConfig] mode=${STEPCOUNT_RL_MODE} adv=${ADV_ESTIMATOR} lr=${ACTOR_LR} max_grad_norm=0.5"
echo "[RunConfig] clip_low=${CLIP_RATIO_LOW} clip_high=${CLIP_RATIO_HIGH} ppo_epochs=1 kl=0.02"
...
mkdir -p "${MONITOR_DIR}"
MONITOR_LOG="${MONITOR_DIR}/monitor_v12Bpfix_${RUN_TS}.log"
MONITOR_JSON="${MONITOR_DIR}/metrics_v12Bpfix_${RUN_TS}.jsonl"
export STOP_FILE="/tmp/v12Bpfix_stop_${RUN_TS}"
rm -f "${STOP_FILE}"
...
if [[ -f "${MONITOR_SCRIPT}" ]]; then
    ...
    nohup python3 "${MONITOR_SCRIPT}" \
        ...
        > /dev/null 2>"${MONITOR_DIR}/monitor_v12Bpfix_stderr_${RUN_TS}.log" &
    MONITOR_PID=$!
...
python3 -m verl.trainer.main \
    ...
    trainer.experiment_name=StepCount-7B-SFT-30k_v12Bpfix_signGD_mixed_${ADV_ESTIMATOR}_$(date +%Y%m%d_%H%M) \
    trainer.logger=['console','wandb'] \
    trainer.save_checkpoint_path=/mnt/shared-storage-user/zhangchenhao/work/EasyR1-latest/save/StepCount-7B-SFT-30k_v12Bpfix_signGD_mixed_${ADV_ESTIMATOR}_$(date +%Y%m%d_%H%M) \
...
echo "[V12-B'  Training finished successfully."
