#!/bin/bash
# ================================================================
# V12-opt Launch Script — StepCount-7B Interleaved Point-to-Count
# ================================================================
# V12-opt = v12 baseline-aligned BoK + stability + format-collapse guard (ppo=1, kl=0.04, BOK_CLIP=3.0, CAP=2.5)
#
# EXPERIMENTAL DESIGN:
...
#
# KEY CONFIG:
#   LR=1.2e-6, clip=0.5, ppo=1, kl=0.04, BOK_CLIP=3.0, CAP_abs=2.5, clip_high=0.24, easy+hard, SFT cold start
...
# V12-fix: SFT cold start, no resume
# v21_RESUME_CKPT=""
...
case "${STEPCOUNT_RL_MODE}" in
  bok_grpo)
    ...
    export GRAD_SPIKE_LR_FACTOR=${GRAD_SPIKE_LR_FACTOR:-0.2}
    export GRAD_SPIKE_BRAKE_WINDOW=${GRAD_SPIKE_BRAKE_WINDOW:-30}
    export GRAD_SPIKE_BRAKE_MAX=${GRAD_SPIKE_BRAKE_MAX:-4}
    export GRAD_SPIKE_ABSOLUTE_CAP=${GRAD_SPIKE_ABSOLUTE_CAP:-2.5}  # v12_opt: tighter abs cap
    export FP16_GRAD_UNDERFLOW_MONITOR=${FP16_GRAD_UNDERFLOW_MONITOR:-0}
    ;;
  *)
    echo "[FATAL] V12-fix only supports bok_grpo mode. Got: ${STEPCOUNT_RL_MODE}" >&2
    exit 1
    ;;
esac
...
LOG_FILE="${LOG_DIR}/training_interleaved_traj_v12opt_${STEPCOUNT_RL_MODE}_${ADV_ESTIMATOR}_${RUN_TS}.log"

echo "================================================================"
echo "[V12-fix RunConfig] Sign-GD + Mixed Data (hard x2) + Confirmed Optimizations"
echo "================================================================"
echo "[RunConfig] model=${MODEL_PATH}"
echo "[RunConfig] mode=${STEPCOUNT_RL_MODE} adv=${ADV_ESTIMATOR} lr=${ACTOR_LR} max_grad_norm=0.5"
echo "[RunConfig] clip_low=${CLIP_RATIO_LOW} clip_high=${CLIP_RATIO_HIGH} ppo_epochs=1 kl=0.04 bok_clip=3.0 abs_cap=2.5"
...
mkdir -p "${MONITOR_DIR}"
MONITOR_LOG="${MONITOR_DIR}/monitor_v12opt_${RUN_TS}.log"
MONITOR_JSON="${MONITOR_DIR}/metrics_v12opt_${RUN_TS}.jsonl"
export STOP_FILE="/tmp/v12opt_stop_${RUN_TS}"
rm -f "${STOP_FILE}"
...
if [[ -f "${MONITOR_SCRIPT}" ]]; then
    ...
    nohup python3 "${MONITOR_SCRIPT}" \
        ...
        > /dev/null 2>"${MONITOR_DIR}/monitor_v12opt_stderr_${RUN_TS}.log" &
    MONITOR_PID=$!
...
python3 -m verl.trainer.main \
    ...
    trainer.experiment_name=StepCount-7B-SFT-30k_v12opt_signGD_mixed_${ADV_ESTIMATOR}_$(date +%Y%m%d_%H%M) \
    trainer.logger=['console','wandb'] \
    trainer.save_checkpoint_path=/mnt/shared-storage-user/zhangchenhao/work/EasyR1-latest/save/StepCount-7B-SFT-30k_v12opt_signGD_mixed_${ADV_ESTIMATOR}_$(date +%Y%m%d_%H%M) \
...
echo "[V12-fix] Training finished successfully."
