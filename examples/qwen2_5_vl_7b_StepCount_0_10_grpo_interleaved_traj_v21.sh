#!/bin/bash
# ================================================================
# v21 Launch Script — StepCount-7B Interleaved Point-to-Count
# ================================================================
...
# v21: Fresh start from SFT (no resume) checkpoint (comment out to start from SFT)
# v21: Uncomment below to resume from a checkpoint (uncomment below to resume from V11 S66)
v21_RESUME_CKPT="${v21_RESUME_CKPT:-}"
# v21_RESUME_CKPT="${v21_RESUME_CKPT:-}"
...
case "${STEPCOUNT_RL_MODE}" in
  ...
  bok_grpo)
    ...
    export GRAD_SPIKE_THRESHOLD=${GRAD_SPIKE_THRESHOLD:-3.5}   # spike if grad_norm > EMA * threshold
...
LOG_FILE="${LOG_DIR}/training_interleaved_traj_v21_StepCount_mask_reward_v4_${STEPCOUNT_RL_MODE}_hm${INTERLEAVED_HISTORY_MODE}_gate${TRAJ_ANSWER_GATE_MODE}_fmtrej${TRAJ_FORMAT_REJECTION}_${ADV_ESTIMATOR}_${RUN_TS}.log"
...
echo "[RunConfig] v21: resume_ckpt=${v21_RESUME_CKPT} ppo_epochs=2 max_grad_norm=1.0 easy_threshold=${BOK_EASY_THRESHOLD:-0.50} easy_score_threshold=${BOK_EASY_SCORE_THRESHOLD:-0.5} filter_all_correct=${BOK_FILTER_ALL_CORRECT:-1}"
...
mkdir -p "${MONITOR_DIR}"
MONITOR_LOG="${MONITOR_DIR}/monitor_v21_${RUN_TS}.log"
MONITOR_JSON="${MONITOR_DIR}/metrics_v21_${RUN_TS}.jsonl"
export STOP_FILE="/tmp/v21_stop_${RUN_TS}"
...
if [[ -f "${MONITOR_SCRIPT}" ]]; then
    ...
    nohup python3 "${MONITOR_SCRIPT}" \
        ...
        > /dev/null 2>"${MONITOR_DIR}/monitor_v21_stderr_${RUN_TS}.log" &
    MONITOR_PID=$!
...
python3 -m verl.trainer.main \
    ...
    trainer.experiment_name=StepCount-7B-SFT-30k_v21_mask_reward_v4${STEPCOUNT_RL_MODE}_hm${INTERLEAVED_HISTORY_MODE}_gate${TRAJ_ANSWER_GATE_MODE}_fmtrej${TRAJ_FORMAT_REJECTION}_${ADV_ESTIMATOR}_KL${DISABLE_KL}_ailab_$(date +%Y%m%d_%H%M) \
    trainer.logger=['console','wandb'] \
    trainer.save_checkpoint_path=/mnt/shared-storage-user/zhangchenhao/work/EasyR1-latest/save/StepCount-7B-SFT-30k_v21_mask_reward_v4${STEPCOUNT_RL_MODE}_hm${INTERLEAVED_HISTORY_MODE}_gate${TRAJ_ANSWER_GATE_MODE}_${ADV_ESTIMATOR}_$(date +%Y%m%d_%H%M) \
