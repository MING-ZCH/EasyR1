#!/bin/bash
# ================================================================
# V14-hard-only Launch Script — Hard-Data-Only — StepCount-7B Interleaved Point-to-Count
# ================================================================
# Key changes from V14 (hard-data-only experiment):
#   1. Training data: hard-only (1808 = 1616 SFT-base wrong + 192 external wrong)
#   2. total_epochs=4 (~113 steps vs V14 full-data 178 steps/epoch)
#   3. BOK_TOTAL_STEPS=113 (28 steps/epoch × 4 epochs)
#   4. save_freq=28 (save every epoch)
#   5. All other params same as V14 (LR=1e-6, BOK_CLIP=3.0, max_grad_norm=1.0)
#   6. NaN hypothesis: hard-data reduces steps + LR=1e-6 should be stable
...
case "${STEPCOUNT_RL_MODE}" in
  ...
  bok_grpo)
    ...
    [[ -z "${PROCESS_REWARD_ENABLE}" ]] && PROCESS_REWARD_ENABLE="0"
    [[ -z "${ACTOR_LR}" ]] && ACTOR_LR="1e-6"  # V14: reduce from 1.5e-6 to 1e-6 (V7 GRPO verified NaN-free at this LR)
    ...
    export BOK_TOTAL_STEPS=${BOK_TOTAL_STEPS:-113} # total_epoch 1 =178
...
LOG_FILE="${LOG_DIR}/training_interleaved_traj_v14_hard_only_StepCount_mask_reward_v4_${STEPCOUNT_RL_MODE}_hm${INTERLEAVED_HISTORY_MODE}_gate${TRAJ_ANSWER_GATE_MODE}_fmtrej${TRAJ_FORMAT_REJECTION}_${ADV_ESTIMATOR}_${RUN_TS}.log"
...
mkdir -p "${MONITOR_DIR}"
WATCHDOG_LOG="${MONITOR_DIR}/watchdog_v14_hard_only_${RUN_TS}.log"
WATCHDOG_JSON="${MONITOR_DIR}/metrics_v14_hard_only_${RUN_TS}.jsonl"
STOP_FILE="/tmp/v14_hard_only_stop_${RUN_TS}"
...
if [[ -f "${WATCHDOG_SCRIPT}" ]]; then
    ...
    nohup python3 "${WATCHDOG_SCRIPT}" \
        ...
        > /dev/null 2>"${MONITOR_DIR}/watchdog_v14_hard_only_stderr_${RUN_TS}.log" &
    WATCHDOG_PID=$!
...
python3 -m verl.trainer.main \
    ...
    data.train_files=/mnt/shared-storage-user/zhangchenhao/work/StepcountModel/dataset/StepCountQA-RL-Traj_0_10_hard_only \
    ...
    trainer.experiment_name=StepCount-7B-SFT-30k_v14_hard_only_mask_reward_v4${STEPCOUNT_RL_MODE}_hm${INTERLEAVED_HISTORY_MODE}_gate${TRAJ_ANSWER_GATE_MODE}_fmtrej${TRAJ_FORMAT_REJECTION}_${ADV_ESTIMATOR}_KL${DISABLE_KL}_ailab_$(date +%Y%m%d_%H%M) \
    trainer.logger=['console','wandb'] \
    trainer.save_checkpoint_path=/mnt/shared-storage-user/zhangchenhao/work/EasyR1-latest/save/StepCount-7B-SFT-30k_v14_hard_only_mask_reward_v4${STEPCOUNT_RL_MODE}_hm${INTERLEAVED_HISTORY_MODE}_gate${TRAJ_ANSWER_GATE_MODE}_${ADV_ESTIMATOR}_$(date +%Y%m%d_%H%M) \
