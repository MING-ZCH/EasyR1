#!/bin/bash
# ================================================================
# V14 Launch Script — StepCount-7B Interleaved Point-to-Count
# ================================================================
# Key changes from V12/V13:
#   1. Start from SFT base model (no resume from checkpoint)
#   2. LR reduced: 1.5e-6 → 1e-6 (V7 GRPO verified NaN-free at this LR)
#   3. max_grad_norm: 0.5 → 1.0 (V12 mean grad_norm=1.35, 0.5 clips too much)
#   4. BOK_CLIP: keep 3.0 (LR reduction is primary fix)
#   5. Reverted V13 ineffective code changes (logits clamp, loss NaN guard)
#   6. total_epochs=2, BOK_TOTAL_STEPS=356
# Root cause of V12/V13 NaN: BoK-GRPO advantage hits clip boundaries every
#   step → large gradients → bf16 overflow at backward pass. Combined with
#   lr=1.5e-6 (+50% vs GRPO baseline), positive feedback loop causes NaN.
# ================================================================
...
LOG_FILE="${LOG_DIR}/training_interleaved_traj_v14_StepCount_mask_reward_v4_${STEPCOUNT_RL_MODE}_hm${INTERLEAVED_HISTORY_MODE}_gate${TRAJ_ANSWER_GATE_MODE}_fmtrej${TRAJ_FORMAT_REJECTION}_${ADV_ESTIMATOR}_${RUN_TS}.log"
...
mkdir -p "${MONITOR_DIR}"
WATCHDOG_LOG="${MONITOR_DIR}/watchdog_v14_${RUN_TS}.log"
WATCHDOG_JSON="${MONITOR_DIR}/metrics_v14_${RUN_TS}.jsonl"
STOP_FILE="/tmp/v14_stop_${RUN_TS}"
...
if [[ -f "${WATCHDOG_SCRIPT}" ]]; then
    ...
    nohup python3 "${WATCHDOG_SCRIPT}" \
        ...
        > /dev/null 2>"${MONITOR_DIR}/watchdog_v14_stderr_${RUN_TS}.log" &
    WATCHDOG_PID=$!
...
python3 -m verl.trainer.main \
    ...
    trainer.experiment_name=StepCount-7B-SFT-30k_v14_mask_reward_v4${STEPCOUNT_RL_MODE}_hm${INTERLEAVED_HISTORY_MODE}_gate${TRAJ_ANSWER_GATE_MODE}_fmtrej${TRAJ_FORMAT_REJECTION}_${ADV_ESTIMATOR}_KL${DISABLE_KL}_ailab_$(date +%Y%m%d_%H%M) \
    trainer.logger=['console','wandb'] \
    trainer.save_checkpoint_path=/mnt/shared-storage-user/zhangchenhao/work/EasyR1-latest/save/StepCount-7B-SFT-30k_v14_mask_reward_v4${STEPCOUNT_RL_MODE}_hm${INTERLEAVED_HISTORY_MODE}_gate${TRAJ_ANSWER_GATE_MODE}_${ADV_ESTIMATOR}_$(date +%Y%m%d_%H%M) \
