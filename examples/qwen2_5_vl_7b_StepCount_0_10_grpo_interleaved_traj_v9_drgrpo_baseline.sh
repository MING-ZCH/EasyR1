...
export STEPCOUNT_RL_MODE=${STEPCOUNT_RL_MODE:-drgrpo}
...
case "${STEPCOUNT_RL_MODE}" in
  ...
  bok_grpo)
    ...
    export BOK_CLIP=${BOK_CLIP:-3.0}   # v8: raised from 3.0→4.0, Hard gradient +13.5%, Easy unchanged
    ...
    export BOK_TAU_INIT=${BOK_TAU_INIT:-0.7}   # start with mild exploration (τ=0.8)
    export BOK_TAU_FINAL=${BOK_TAU_FINAL:-0.5}  # anneal to selective (low τ); avoid <0.5 to preserve format compliance
...
export ANSWER_WEIGHT=${ANSWER_WEIGHT:-0.8}
export POINT_WEIGHT=${POINT_WEIGHT:-0.1}
...
MODEL_PATH=/mnt/shared-storage-user/work/StepcountModel/model/StepCount-7B-SFT-30k-high/checkpoint-3537
...
LOG_FILE="${LOG_DIR}/training_interleaved_traj_v9_drgrpo_baseline_StepCount_mask_reward_v4_${STEPCOUNT_RL_MODE}_hm${INTERLEAVED_HISTORY_MODE}_gate${TRAJ_ANSWER_GATE_MODE}_fmtrej${TRAJ_FORMAT_REJECTION}_${ADV_ESTIMATOR}_${RUN_TS}.log"
...
python3 -m verl.trainer.main \
    ...
    trainer.experiment_name=StepCount-7B-SFT-30k_v9_drgrpo_baseline_mask_reward_v4${STEPCOUNT_RL_MODE}_hm${INTERLEAVED_HISTORY_MODE}_gate${TRAJ_ANSWER_GATE_MODE}_fmtrej${TRAJ_FORMAT_REJECTION}_${ADV_ESTIMATOR}_KL${DISABLE_KL}_ailab_$(date +%Y%m%d_%H%M) \
    trainer.logger=['console','wandb'] \
    trainer.save_checkpoint_path=/mnt/shared-storage-user/work/EasyR1-latest/save/StepCount-7B-SFT-30k_v9_drgrpo_baseline_mask_reward_v4${STEPCOUNT_RL_MODE}_hm${INTERLEAVED_HISTORY_MODE}_gate${TRAJ_ANSWER_GATE_MODE}_${ADV_ESTIMATOR}_$(date +%Y%m%d_%H%M) \
    trainer.save_freq=132 \
