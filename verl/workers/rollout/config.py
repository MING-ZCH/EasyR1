...
class RolloutConfig:
    ...
    interleaved_adaptive_max_turns: bool = False
    interleaved_adaptive_max_turns_margin: int = 2
    interleaved_point_turn_use_answer_budget: bool = False
    ...
    # rollout.n stays invariant because trainer/GRPO group and gradient
    # accumulation semantics are configured from it. Auto-protect may enable
    # chunked prefill, but never silently changes n inside a step.
    auto_protect: bool = True
    # Deprecated compatibility key; retained for old Hydra overrides.
