...
# H200 should not enable this by default; the launcher clears these env vars for
# the h200 profile and this guard avoids PyTorch experimental-feature warnings.
...
# Apply TF32 settings early (before any cuBLAS call) only for the H20 workaround.
if _H20_SIGFPE_WORKAROUND:
...
def _normalize_sample_max_turns(
    sample_max_turns: Optional[Any],
    base_count: int,
    repeat_times: int,
    global_max_turns: int,
) -> List[int]:
    global_cap = max(1, int(global_max_turns))
    repeated_caps: List[int] = []
    values = list(sample_max_turns) if sample_max_turns is not None else []
    for sample_idx in range(base_count):
        cap = global_cap
        if sample_idx < len(values):
            try:
                cap = int(values[sample_idx])
            except (TypeError, ValueError):
                cap = global_cap
        cap = max(1, min(global_cap, cap))
        repeated_caps.extend([cap] * max(1, int(repeat_times)))
    return repeated_caps
...
class vLLMRollout(BaseRollout):
    ...
    def _generate_interleaved_responses(
        ...
        sample_max_turns: Optional[Any] = None,
        ...
        per_sample_max_turns = _normalize_sample_max_turns(
            sample_max_turns=sample_max_turns,
            base_count=len(vllm_inputs),
            repeat_times=repeat_times,
            global_max_turns=max_turns,
        )
        effective_loop_max_turns = max(per_sample_max_turns) if per_sample_max_turns else max_turns
        ...
        with self.update_sampling_params(n=1):
            ...
            for turn_idx in range(effective_loop_max_turns):
                turn_indices = [
                    idx
                ...
                normal_point_indices = [idx for idx in normal_indices if turn_idx < per_sample_max_turns[idx] - 1]
                normal_final_indices = [idx for idx in normal_indices if turn_idx >= per_sample_max_turns[idx] - 1]
                ...
                if debug_enabled and self.rank == 0:
                    print(
                        "[InterleavedDebug] "
                        f"step={step_info} turn={turn_idx + 1}/{effective_loop_max_turns} active={len(turn_indices)} "
                        f"normal={len(normal_indices)} normal_final={len(normal_final_indices)} "
                        f"answer_mode={len(answer_indices)}"
                    )
                ...
                for global_idx, output in completion_pairs:
                    ...
                    if stopped_on_point_close or stopped_on_answer_close or stopped_on_answer_open:
                        try:
                            ...
                            decoded_before_stop = ""
                        if stopped_on_point_close and point_close_tag not in decoded_before_stop:
                            token_ids.extend(point_close_token_ids)
                            decoded_before_stop += point_close_tag
                        if stopped_on_answer_close and stop_tag not in decoded_before_stop:
                            token_ids.extend(answer_close_token_ids)
                            decoded_before_stop += stop_tag
                        if stopped_on_answer_open and answer_open_tag not in decoded_before_stop:
                    ...
                    for parsed_point in parsed_points:
                        ...
                        expected_count = point_counts[global_idx] + 1
                        if parsed_count is not None and parsed_count > 0 and parsed_count != expected_count:
                            turn_count_number_mismatch += 1
                        # The image annotation count is local rollout state, not a
                        # value the model can jump via count_number.
                        point_counts[global_idx] = expected_count
                ...
                if debug_enabled and self.rank == 0:
                    print(
                        ...
                        f"count_number_mismatch={turn_count_number_mismatch} "
    ...
    def generate_sequences(self, prompts: DataProto) -> DataProto:
        ...
        adaptive_max_turns = non_tensor_batch.pop("adaptive_max_turns", None)
        ...
        with self.update_sampling_params(**prompts.meta_info):
            ...
            # Keep n fixed for the entire trainer step. GRPO/BoK worker batch sizes,
            # gradient accumulation, UID groups, and advantage normalization are
            # initialized from rollout.n; changing n inside generation would make
            # those contracts inconsistent. Resource fallback must restart from a
            # checkpoint with an explicitly smaller ROLLOUT_N.
            actual_n = desired_n
            ...
            if self.config.interleaved_point_to_count and has_multi_modal:
                ...
                response_ids, action_event_ledger = self._generate_interleaved_responses(
                    ...
                    sample_max_turns=adaptive_max_turns,
                ...
                while retries <= int(self.config.auto_protect_max_retries):
                    try:
                        ...
                        # Keep rollout.n invariant; only retry with chunked prefill.
