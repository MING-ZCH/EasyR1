...
class RayPPOTrainer:
    ...
    def _validate(self) -> Dict[str, Any]:
        ...
        try:
            ...
            print(
                "[ValTaskMix] "
                f"samples={len(sample_scores)} point_rate={point_rate:.3f} count_rate={count_rate:.3f} traj_rate={traj_rate:.3f}"
            )
        except Exception:
            pass

        val_reward_metrics = {f"val/{key}_reward": value for key, value in reduced.items()}
        # Structured single-line validation summary for automated parsing
        _val_result = {"val/reward_score": reward_score, **val_reward_metrics}
        _val_parts = " ".join(f"{k}={v:.4f}" for k, v in sorted(_val_result.items()) if isinstance(v, (int, float)))
        print(f"[ValSummary] step={getattr(self, 'global_step', -1)} {_val_parts}")
        return _val_result
